...
class HTMLEditorApp(Adw.Application):
    def __init__(self, **kwargs):
        ...
        # Add formatting state tracking to the application
        self.is_bold = False
    ...
    def create_window(self):
        ...
        # Add formatting state tracking to the window
        win.is_bold = False
        win.is_italic = False
        win.is_underline = False
...
#############
    def on_bold_toggled(self, win, button):
        ...
        try:
            ...
            win.webview.evaluate_javascript(
                "document.queryCommandState('bold');",
                -1, None, None, None,
                lambda webview, result, data: self._on_bold_state_updated(win, webview, result),
                None
            )
        finally:
            # Set a small delay before clearing the processing flag
            GLib.timeout_add(100, self._clear_bold_processing)
        
    def _clear_bold_processing(self):
        self._processing_bold_toggle = False
        return False  # Don't call again
        
    def _on_bold_state_updated(self, win, webview, result):
        """Update the bold button state after querying the current state"""
        try:
            js_result = webview.evaluate_javascript_finish(result)
            
            if js_result:
                if hasattr(js_result, 'get_js_value'):
                    current_state = js_result.get_js_value().to_boolean()
                elif hasattr(js_result, 'to_boolean'):
                    current_state = js_result.to_boolean()
                else:
                    current_state = bool(js_result)
            else:
                current_state = win.bold_button.get_active()
            
            # Store state and update button
            win.is_bold = current_state
            
            # Update button state
            if win.bold_handler_id is not None:
                win.bold_button.handler_block(win.bold_handler_id)
                win.bold_button.set_active(current_state)
                win.bold_button.handler_unblock(win.bold_handler_id)
                
            # Set focus back to webview
            win.webview.grab_focus()
        except Exception as e:
            print(f"Error updating bold state: {e}")
    ...
    def on_italic_toggled(self, win, button):
        ...
        try:
            ...
            win.webview.evaluate_javascript(
                "document.queryCommandState('italic');",
                -1, None, None, None,
                lambda webview, result, data: self._on_italic_state_updated(win, webview, result),
                None
            )
        finally:
            # Set a small delay before clearing the processing flag
            GLib.timeout_add(100, self._clear_italic_processing)
        
    def _clear_italic_processing(self):
        self._processing_italic_toggle = False
        return False  # Don't call again
        
    def _on_italic_state_updated(self, win, webview, result):
        """Update the italic button state after querying the current state"""
        try:
            js_result = webview.evaluate_javascript_finish(result)
            ...
            if js_result:
                ...
                current_state = win.italic_button.get_active()
            
            # Store state and update button
            win.is_italic = current_state
            
            # Update button state
            if win.italic_handler_id is not None:
                win.italic_button.handler_block(win.italic_handler_id)
                win.italic_button.set_active(current_state)
                win.italic_button.handler_unblock(win.italic_handler_id)
                
            # Set focus back to webview
            win.webview.grab_focus()
        except Exception as e:
            print(f"Error updating italic state: {e}")
    ...
    def on_underline_toggled(self, win, button):
        ...
        try:
            # First check the current state to determine what we should do
            win.webview.evaluate_javascript(
                "document.queryCommandState('underline');",
                -1, None, None, None,
                lambda webview, result, data: self._process_underline_toggle(win, webview, result),
                None
            )
        finally:
            # Set a small delay before clearing the processing flag
            GLib.timeout_add(100, self._clear_underline_processing)
        
    def _clear_underline_processing(self):
        self._processing_underline_toggle = False
        return False  # Don't call again

    def _process_underline_toggle(self, win, webview, result):
        try:
            js_result = webview.evaluate_javascript_finish(result)
            ...
            if js_result:
                ...
                current_state = win.underline_button.get_active()
            
            # Execute the command to toggle the state
            self.execute_js(win, "document.execCommand('underline', false, null)")
            
            # Update our own tracking state
            win.is_underline = not current_state
            
            # Update button UI state
            if win.underline_handler_id is not None:
                win.underline_button.handler_block(win.underline_handler_id)
                win.underline_button.set_active(not current_state)
                win.underline_button.handler_unblock(win.underline_handler_id)
            
            # Set focus back to webview
            win.webview.grab_focus()
            
            # Set a window variable to track underline state specifically
            self.execute_js(win, f"window.editorUnderlineActive = {str(not current_state).lower()};")
            
        except Exception as e:
            print(f"Error processing underline toggle: {e}")

    # Replace the keyboard shortcut handlers
    def on_bold_shortcut(self, win, *args):
        """Handle Ctrl+B shortcut for bold formatting"""
        if hasattr(self, '_processing_bold_toggle') and self._processing_bold_toggle:
            return True
        self.on_bold_toggled(win, win.bold_button)
        return True
    ...
    def on_italic_shortcut(self, win, *args):
        ...
        if hasattr(self, '_processing_italic_toggle') and self._processing_italic_toggle:
            return True
        self.on_italic_toggled(win, win.italic_button)
        return True
    ...
    def on_underline_shortcut(self, win, *args):
        ...
        if hasattr(self, '_processing_underline_toggle') and self._processing_underline_toggle:
            return True
        self.on_underline_toggled(win, win.underline_button)
        return True

    # Special handling for selection change events
    def selection_change_js(self):
        """JavaScript to track selection changes and update formatting buttons"""
        return """
        // Track formatting state
        window.editorUnderlineActive = false;
        window.lastNodeBeforeEnter = null;
        ...
        function updateFormattingState() {
            try {
                ...
                // Special handling for underline after Enter
                let isUnderline = document.queryCommandState('underline');
                
                // Store current node for Enter key handling
                window.lastNodeBeforeEnter = element;
    ...
    # Replace the on_formatting_changed method
    def on_formatting_changed(self, win, manager, result):
        """Update toggle button states based on current formatting"""
        try:
            # Extract the message differently based on WebKit version
            ...
            if win.underline_handler_id is not None:
                win.underline_button.handler_block(win.underline_handler_id)
                # Use our tracked state when available
                win.underline_button.set_active(format_state.get('underline', False))
                win.underline_button.handler_unblock(win.underline_handler_id)
