    ...
    def apply_persistent_formatting(self, format_type, enable):
        ...
        script = f"""
            (function() {{
                ...
                }} else if ('{format_type}' === 'underline' || '{format_type}' === 'strikethrough') {{
                    // For text decorations, we handle them together to prevent conflicts
                    if ({str(enable).lower()} !== document.queryCommandState('{format_type}')) {{
                        document.execCommand('{format_type}', false, null);
                    }}
                }}

                if (range.collapsed) {{
                    // Create a span to hold formatting for future typing
