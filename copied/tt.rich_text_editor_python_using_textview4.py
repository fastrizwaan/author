...
class ResizableWidget(Gtk.Overlay):
    def __init__(self, child, min_width=50, min_height=50, pixbuf=None):
        ...
        for name, x_align, y_align in positions:
            ...
            handle.set_draw_func(self.draw_resize_handle)
        ...
        # Toggle handles with click (only on the child content)
        click = Gtk.GestureClick()
        click.connect("pressed", self.on_click)
        self.child.add_controller(click)  # Apply click to child, not overlay
        ...
        self.child.add_controller(self.drag_gesture)  # Apply drag to child
...
class TextEditor(Adw.ApplicationWindow):
    ...
    def on_insert_text_clicked(self, button):
        dialog = Gtk.MessageDialog(
            ...
            secondary_text="Enter the text for the resizable text box:"
        )
        entry = Gtk.Entry()
    ...
    def on_insert_text_response(self, dialog, response, entry):
        if response == Gtk.ResponseType.OK:
            ...
            if text:
                ...
                resizable_box = ResizableWidget(text_entry, min_width=100, min_height=30)
