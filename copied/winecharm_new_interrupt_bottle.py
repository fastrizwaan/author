...
############################# CREate bottle 2 with interruption
    def create_bottle(self, script, script_key, backup_path):
        """
        Backs up the Wine prefix in a stepwise manner, indicating progress via spinner and label updates.
        """
        ...
        # Step 1: Disconnect the UI elements and initialize the spinner
        self.show_processing_spinner("Bottling...")
        self.connect_open_button_with_bottling_cancel()
    ...
    def connect_open_button_with_bottling_cancel(self):
        """
        Disconnect the open button's handler and connect cancel handler
        """
        if self.open_button_handler_id is not None:
            self.open_button.disconnect(self.open_button_handler_id)
            # Connect the cancel handler
    ...
    def on_cancel_bottle_dialog_response(self, dialog, response):
        ...
        if response == "cancel":
            self.stop_processing = True
        dialog.close()
################################# CREATE BOTTLE 3 with interruption works
    def create_bottle(self, script, script_key, backup_path):
        """
        Backs up the Wine prefix in a stepwise manner, indicating progress via spinner and label updates.
        """
