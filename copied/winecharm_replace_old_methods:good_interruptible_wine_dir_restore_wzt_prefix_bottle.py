...
##### /BACKUP PREFIX xx
        ...
        try:
            ...
            if file:
                ...
                if file_path.endswith(".prefix") or file_path.endswith(".bottle"):
                    ...
                    self.rxestore_wzt_backup(file_path)
    ...
    def rxestore_wzt_backup(self, file_path):
        """
...
############################################### 4444444444444444444444444 New initialize template
    ...
    def restore_prefix_bottle_wzt_tar_zst(self, file_path):
        """
        Restore from a .prefix or .bottle tar.zst file with interruption support.
        """
        self.stop_processing = False
        
        # Clear the flowbox and show progress
        GLib.idle_add(self.flowbox.remove_all)
        self.show_processing_spinner(f"Restoring from {Path(file_path).name}")
        self.disconnect_open_button()
        
        # Extract prefix name from the archive
        prefix_name = Path(file_path).stem
        dest_dir = self.prefixes_dir / prefix_name
        #temp_dir = dest_dir.parent / f"{prefix_name}_temp_{int(time.time())}"
        temp_dir = dest_dir
        
        # Define restore steps
        steps = [
            ("Checking Disk Space", lambda: self.check_disk_space_and_show_step(file_path)),
            ("Extracting Archive", lambda: self.extract_tar_to_temp(file_path, temp_dir)),
            ("Performing Replacements", lambda: self.perform_replacements(temp_dir)),
            ("Processing Registry Files", lambda: self.process_reg_files(temp_dir)),
            ("Replacing Symbolic Links", lambda: self.remove_symlinks_and_create_directories(temp_dir)),
            ("Renaming User Directories", lambda: self.rename_and_merge_user_directories(temp_dir)),
            ("Creating Scripts", lambda: self.create_scripts_for_exe_files(temp_dir))
        ]
        
        # Set total steps and connect cancel button
        self.total_steps = len(steps)
        self.connect_open_button_with_restore_backup_cancel()
        
        def perform_restore_steps():
            try:
                for index, (step_text, step_func) in enumerate(steps, 1):
                    if self.stop_processing:
                ...
                if not self.stop_processing:
                    # Success - handle existing directory if present
                    if dest_dir.exists():
                        # Show overwrite confirmation dialog
                        GLib.idle_add(self.show_restore_overwrite_confirmation_dialog, temp_dir, dest_dir)
                    else:
                ...
                GLib.idle_add(self.cleanup_cancelled_restore, temp_dir)
                GLib.idle_add(self.show_info_dialog, "Error", f"Restore failed: {e}")

        threading.Thread(target=perform_restore_steps).start()
    ...
    def extract_wzt_to_temp(self, wzt_file, temp_dir):
        ...
        try:
            subprocess.run(
                ["tar", "-xvf", wzt_file, "-C", str(temp_dir), "--transform", f"s|XOUSERXO|{user}|g"],
                check=True
            )
            print(f"Successfully extracted to {temp_dir}")
            return True
...
###################################3 55555555555555555555 initiliaze
    ...
    def get_wzt_restore_steps(self, file_path):
        ...
        return [
            ...
            ("Performing Replacements", lambda: self.perform_replacements(self.extract_prefix_dir(file_path))),
            ("Processing Shell Files", lambda: self.process_sh_files(self.extract_prefix_dir(file_path))),
            ("Replacing Symbolic Links with Directories", lambda: self.remove_symlinks_and_create_directories(self.extract_prefix_dir(file_path))),
            ("Renaming and merging user directories", lambda: self.rename_and_merge_user_directories(self.extract_prefix_dir(file_path))),
            ("Finding and Saving LNK Files", lambda: self.find_and_save_lnk_files(self.extract_prefix_dir(file_path))),
        ]
...
########################33 simplify extract wzt and wzt steps
    def extract_wzt_file(self, wzt_file):
        """
        Extract the .wzt file to the Wine prefixes directory and process the files.
        """
