...
<body><p>&nbsp;</p></body>
        ...
        insert_group.add_css_class("toolbar-group")
        ...
        formatting_toolbar_group.append(insert_group)
        ...
        self.textbox_btn = Gtk.Button(icon_name="insert-object")
        self.textbox_btn.add_css_class("flat")
        self.textbox_btn.connect("clicked", self.on_insert_textbox_clicked)
        insert_group.append(self.textbox_btn)

        # Insert Image Button
        self.image_btn = Gtk.Button(icon_name="insert-image")
        self.image_btn.add_css_class("flat")
        self.image_btn.connect("clicked", self.on_insert_image_clicked)
        insert_group.append(self.image_btn)
    ...
    def on_insert_image_clicked(self, btn):
        ...
        file_dialog.open(self, None, self.on_image_file_selected)

    def on_image_file_selected(self, dialog, result):
        try:
            ...
            print("Image selection error:", e.message)
    ...
    def on_webview_load(self, webview, load_event):
        if load_event == WebKit.LoadEvent.FINISHED:
            ...
            # Enhanced script with anchoring and wrapping
            enhanced_script = """
                (function() {
                    ...
                    // Add styles for textboxes and images
                    let style = document.createElement('style');
                    style.textContent = `
                        .textbox {
                        ...
                        .textbox:focus, .resizable-image:focus {
                        ...
                        .textbox.flow-over, .resizable-image.flow-over {
                            position: absolute;
                            z-index: 0; /* Below text */
                        }
                        .textbox.flow-above, .resizable-image.flow-above {
                            position: absolute;
                    ...
                    function makeElementInteractive(element) {
                        ...
                        // Create an anchor point if not already present
                        if (!element.previousElementSibling || !element.previousElementSibling.classList.contains('anchor-point')) {
                        ...
                        function onMouseMove(e) {
                            ...
                            if (isResizing) {
                                ...
                                if (element.classList.contains('resizable-image')) {
                                    const aspectRatio = parseFloat(element.dataset.aspectRatio) || (element.naturalWidth / element.naturalHeight);
                        ...
                        element.startDragging = function(e) {
                            if (element.style.position === 'absolute') {
                                isDragging = true;
                                startX = e.clientX;
                                startY = e.clientY;
                        ...
                        element.startResizing = function(e) {
                            isResizing = true;
                            startX = e.clientX;
                            startY = e.clientY;
                            ...
                            if (element.classList.contains('resizable-image')) {
                                element.dataset.aspectRatio = startWidth / startHeight;
                            }
                            document.addEventListener('mousemove', onMouseMove);
                            document.addEventListener('mouseup', onMouseUp);
                        };
                    ...
                    // Apply interactivity to existing elements
                    function setupExistingElements() {
                        document.querySelectorAll('.textbox, .resizable-image').forEach(element => {
                        ...
                        mutations.forEach(function(mutation) {
                            if (mutation.type === 'childList') {
                                mutation.addedNodes.forEach(function(node) {
                                    if (node.nodeType === 1) {
                                        if (node.classList && 
                                            (node.classList.contains('textbox') || 
                                            node.classList.contains('resizable-image'))) {
                                            makeElementInteractive(node);
                                        }
                                        node.querySelectorAll('.textbox, .resizable-image').forEach(element => {
                                            makeElementInteractive(element);
                                        });
                                    }
                                });
                    ...
                    document.body.addEventListener('contextmenu', function(e) {
                        const closestElement = e.target.closest('.textbox, .resizable-image');
                        ...
                        if (closestElement || 
                            ...
                            activeElement = closestElement || e.target;
                            
                            let menu = document.createElement('div');
                            menu.className = 'context-menu';
                            menu.innerHTML = `
                                <div id="menu-resize">Resize</div>
                                <div id="menu-move">Move</div>
                                <div id="menu-bring-front">Bring to Front</div>
                                <div id="menu-send-back">Send to Back</div>
                                <div id="menu-border-none">No Border</div>
                                <div id="menu-border-solid">Solid Border</div>
                                <div id="menu-border-dashed">Dashed Border</div>
                                <div id="menu-text-over">Text Over</div>
                                <div id="menu-text-around-left">Text Around (Left)</div>
                                <div id="menu-text-around-right">Text Around (Right)</div>
                                <div id="menu-above-text">Above Text</div>
                            `;
                            
                            if (activeElement.classList.contains('textbox')) {
                                menu.innerHTML += `
                                    <div id="menu-toggle-editing">Toggle Editing</div>
                                `;
                            }
                            ...
                            menu.querySelector('#menu-resize').addEventListener('click', () => triggerResize(activeElement));
                            menu.querySelector('#menu-move').addEventListener('click', () => triggerMove(activeElement));
                            menu.querySelector('#menu-bring-front').addEventListener('click', () => adjustZIndex(activeElement, 'front'));
                            menu.querySelector('#menu-send-back').addEventListener('click', () => adjustZIndex(activeElement, 'back'));
                            menu.querySelector('#menu-border-none').addEventListener('click', () => setBorder(activeElement, 'none'));
                            menu.querySelector('#menu-border-solid').addEventListener('click', () => setBorder(activeElement, 'solid'));
                            menu.querySelector('#menu-border-dashed').addEventListener('click', () => setBorder(activeElement, 'dashed'));
                            menu.querySelector('#menu-text-over').addEventListener('click', () => setTextFlow(activeElement, 'over'));
                            menu.querySelector('#menu-text-around-left').addEventListener('click', () => setTextFlow(activeElement, 'around-left'));
                            menu.querySelector('#menu-text-around-right').addEventListener('click', () => setTextFlow(activeElement, 'around-right'));
                            menu.querySelector('#menu-above-text').addEventListener('click', () => setTextFlow(activeElement, 'above'));
                            
                            if (activeElement.classList.contains('textbox')) {
                                menu.querySelector('#menu-toggle-editing').addEventListener('click', () => toggleEditing(activeElement));
                            }
                    ...
                    function triggerResize(element) {
                        const event = new MouseEvent('mousedown', {
                            bubbles: true,
                            cancelable: true,
                            clientX: parseInt(element.style.left) + parseInt(element.style.width),
                            clientY: parseInt(element.style.top) + parseInt(element.style.height)
                        });
                        element.startResizing(event);
                    }

                    function triggerMove(element) {
                        const event = new MouseEvent('mousedown', {
                            bubbles: true,
                            cancelable: true,
                            clientX: parseInt(element.style.left) + 10,
                            clientY: parseInt(element.style.top) + 10
                        });
                        element.startDragging(event);
                    }

                    function adjustZIndex(element, direction) {
                        let currentZ = parseInt(window.getComputedStyle(element).zIndex) || 10;
                        if (direction === 'front') {
                            element.style.zIndex = currentZ + 10;
                        } else {
                            element.style.zIndex = Math.max(0, currentZ - 10);
                        }
                        notifyChange();
                    }
                    
                    function setBorder(element, style) {
                        if (style === 'none') {
                    ...
                    function setTextFlow(element, flow) {
                        let anchor = element.previousElementSibling && element.previousElementSibling.classList.contains('anchor-point') 
                            ? element.previousElementSibling 
                            : null;
                        let wrapper = element.parentElement.classList.contains('flow-wrapper') ? element.parentElement : null;
                        ...
                        // Reset styles
                        element.classList.remove('flow-over', 'flow-above');
                        element.style.position = 'absolute';
                        element.style.zIndex = '10';
                        element.style.float = 'none';
                        
                        if (flow === 'over') {
                            element.classList.add('flow-over');
                            element.style.zIndex = '0';
                        } else if (flow === 'around-left' || flow === 'around-right') {
                            // Create or use anchor point
                            if (!anchor) {
                            ...
                            if (flow === 'around-right') {
                                wrapper.classList.add('right');
                            }
                            element.style.position = 'static';
                            anchor.parentNode.insertBefore(wrapper, anchor.nextSibling);
                            wrapper.appendChild(element);
                            anchor.remove(); // Clean up anchor after positioning
                        } else if (flow === 'above') {
                            element.classList.add('flow-above');
                    ...
                    // Modify the textbox insertion code
                    window.createTextbox = function() {
                        ...
                        if (sel.rangeCount > 0) {
                            ...
                            anchor.parentNode.insertBefore(textbox, anchor.nextSibling);
                        } else {
                            document.body.appendChild(textbox);
                        }
                        makeElementInteractive(textbox);
                        notifyChange();
                    };
                })();
            """
            self.webview.evaluate_javascript(enhanced_script, -1, None, None, None, None, None)
