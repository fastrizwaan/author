...
###################### 0.95
    def create_script_row(self, script_key, script_data):
        ...
        if self.icon_view:
            ...
            # Options button (initially hidden)
            options_button = Gtk.Button(icon_name="emblem-system-symbolic", tooltip_text="Options")
            options_button.add_css_class("flat")
            ...
            self.script_ui_data[script_key] = {
                ...
                'play_button': play_button,
                'options_button': options_button,
                'label_box': label_box,  # Store the label box instead of a separate button/label box
                'button_box': top_box,   # Use top_box as the button container
                'label_button_box': None,  # Not needed for this layout
                'is_running': False,
                'script_path': script,
    ...
    def toggle_overlay_buttons(self, script_key):
        ...
        for key, ui in self.script_ui_data.items():
            if key != script_key and ui.get('showing_buttons', False):
                if self.icon_view:
                    ...
                    ui['options_button'].set_opacity(0)
                    ui['options_button'].set_sensitive(False)
                    ui['label_box'].set_opacity(1)  # Show label
                    ui['label_box'].set_sensitive(True)
                else:
        ...
        if self.icon_view:
            if showing_buttons:
                # Hide buttons, show label
                ui['play_button'].set_opacity(0)
                ui['play_button'].set_sensitive(False)
                ui['options_button'].set_opacity(0)
                ui['options_button'].set_sensitive(False)
                ui['label_box'].set_opacity(1)
                ui['label_box'].set_sensitive(True)
                ui['showing_buttons'] = False
                ui['row'].remove_css_class("blue")
            else:
                # Show buttons, hide label
                ...
                ui['label_box'].set_opacity(0)
                ui['label_box'].set_sensitive(False)
...
####################################
    def create_script_row(self, script_key, script_data):
        ...
        if self.icon_view:
            ...
            # Bottom: Label (always visible, not toggled)
            label_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=0)
            label_box.set_halign(Gtk.Align.CENTER)
            main_label = Gtk.Label()
            main_label.set_max_width_chars(25)
            ...
            self.script_ui_data[script_key] = {
                ...
                'play_button': play_button,
                'options_button': options_button,
                'label_box': label_box,  # Store the label box, but it won't toggle
                'button_box': top_box,   # Use top_box as the button container
                'label_button_box': None,  # Not needed for this layout
                'is_running': False,
                'script_path': script,
