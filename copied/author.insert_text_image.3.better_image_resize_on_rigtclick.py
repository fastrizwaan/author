...
<body><p>&nbsp;</p></body>
    ...
    def on_webview_load(self, webview, load_event):
        if load_event == WebKit.LoadEvent.FINISHED:
            ...
            # Enhanced script with corrected toggle text flow
            enhanced_script = """
                (function() {
                    ...
                    // Add styles for textboxes and images with improved resizing controls
                    let style = document.createElement('style');
                    style.textContent = `
                        .textbox {
                        ...
                        .textbox.floating {
                            float: left;
                            position: absolute; /* Keep absolute for dragging/resizing */
                        }
                    `;
                    document.head.appendChild(style);
                    ...
                    function makeElementDraggableAndResizable(element) {
                        // Add resize handle to both images and textboxes
                        const resizeHandle = document.createElement('div');
                        resizeHandle.className = 'resize-handle';
                        element.appendChild(resizeHandle);
                        
                        // For textboxes, add a drag handle at the top
                        if (element.classList.contains('textbox')) {
                            const dragHandle = document.createElement('div');
                            dragHandle.className = 'textbox-handle';
                        ...
                        // Define drag handlers based on element type
                        const dragHandler = element.classList.contains('textbox') ? 
                                            element.querySelector('.textbox-handle') : element;
                        
                        // Textbox drag functionality via the drag handle
                        if (element.classList.contains('textbox')) {
                            dragHandler.addEventListener('mousedown', function(e) {
                                ...
                                // Add events to document to handle movement
                        ...
                        // Image drag functionality (whole element)
                        else if (element.classList.contains('resizable-image')) {
                        ...
                        // Resize functionality for both elements
                        ...
                        function onMouseMove(e) {
                            if (isDragging) {
                                ...
                                // Ensure element stays within window bounds
                            ...
                            if (isResizing) {
                                ...
                                // Handle image vs textbox resizing differently
                                if (element.classList.contains('resizable-image')) {
                                    // Maintain aspect ratio for images
                                    const aspectRatio = parseFloat(element.dataset.aspectRatio) || 1;
                                    newHeight = newWidth / aspectRatio;
                                } else {
                                    // For textboxes, allow free resizing
                    ...
                    // Apply draggable and resizable to existing elements
                            ...
                            if (mutation.type === 'childList') {
                                mutation.addedNodes.forEach(function(node) {
                                    if (node.nodeType === 1) { // Element node
                                        ...
                                        // Check for elements inside the added node
                    ...
                    // Context menu functionality with Resize and Move options
                    ...
                    document.body.addEventListener('contextmenu', function(e) {
                        ...
                        if (closestElement || 
                            ...
                            menu.innerHTML = `
                                ...
                                <div id="menu-align-left">Align Left</div>
                                <div id="menu-align-center">Align Center</div>
                                <div id="menu-align-right">Align Right</div>
                            ...
                            if (activeElement.classList.contains('textbox')) {
                                menu.innerHTML += `<div id="menu-toggle-flow">Toggle Text Flow</div>`;
                            }
                            ...
                            menu.querySelector('#menu-align-left').addEventListener('click', () => alignElement(activeElement, 'left'));
                            menu.querySelector('#menu-align-center').addEventListener('click', () => alignElement(activeElement, 'center'));
                            menu.querySelector('#menu-align-right').addEventListener('click', () => alignElement(activeElement, 'right'));
                            ...
                            if (activeElement.classList.contains('textbox')) {
                                menu.querySelector('#menu-toggle-flow').addEventListener('click', () => toggleTextFlow(activeElement));
                            }
                    ...
                    function triggerMove(element) {
                        ...
                        if (dragHandler) {
                            ...
                            dragHandler.dispatchEvent(event);
                        }
                    }

                    function alignElement(element, align) {
                        if (align === 'left') {
                            ...
                            let parentWidth = document.body.clientWidth;
                            let elementWidth = element.offsetWidth;
                            element.style.left = (parentWidth - elementWidth) + 'px';
                        }
                        notifyChange();
                    }

                    function toggleTextFlow(element) {
                    ...
                    function adjustZIndex(element, direction) {
                        ...
                        if (direction === 'front') {
                            ...
                            element.style.zIndex = Math.max(1, currentZ - 5);
                        }
                        notifyChange();
                    }
                    ...
                    // Modify the textbox insertion code to ensure proper initialization
                    window.createTextbox = function() {
                        ...
                        textbox.style.top = '50px';
                        document.body.appendChild(textbox);
                        makeElementDraggableAndResizable(textbox);
                        notifyChange();
                    };
                })();
            """
