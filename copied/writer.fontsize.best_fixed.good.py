...
<body><p></p></body>
    ...
    def apply_font_and_size_to_typing(self):
        """
        Applies the current font family and size to the selected text or sets them for new text input.
        Executes JavaScript in the WebView to manipulate the DOM, ensuring no nested <span> tags.
        """
        script = f"""
            (function() {{
                // Helper function to remove existing <span> tags with only font styles within the range
                function removeFontSpansInRange(range) {{
                        ...
                        {{
                            acceptNode: function(node) {{
                                if (node.tagName === 'SPAN' && 
                                    ...
                                    node.style.fontSize && 
                                    Object.keys(node.style).length === 2) {{
                    ...
                    while (walker.nextNode()) {{
                        if (range.intersectsNode(walker.currentNode)) {{
                            spansToRemove.push(walker.currentNode);
                        }}
                    }}
                ...
                // Helper function to get all text nodes within the range for complex selections
                ...
                // Get current font settings from Python
                ...
                if (range.collapsed) {{
                    // No selection: Set styles for text to be typed
                    ...
                    // Selection exists: Remove existing font spans and apply new styles
                    removeFontSpansInRange(range);
...
######################################### fix typing span?
    def apply_font_and_size_to_typing(self):
        script = f"""
            (function() {{
                // Helper function to remove existing font spans within a range
                function removeFontSpansInRange(range) {{
                    let walker = document.createTreeWalker(
                        range.commonAncestorContainer,
                        NodeFilter.SHOW_ELEMENT,
                ...
                // Helper function to get text nodes in range for fallback
                function getTextNodesInRange(range) {{
                    let walker = document.createTreeWalker(
                        range.commonAncestorContainer,
                        NodeFilter.SHOW_TEXT,
                ...
                if (range.collapsed) {{
                    // Check if cursor is inside an existing span with font styles
                    let node = range.startContainer;
                    let parentSpan = (node.nodeType === Node.TEXT_NODE ? node.parentElement : node);
                    if (parentSpan.tagName === 'SPAN' && 
                        (parentSpan.style.fontFamily || parentSpan.style.fontSize) && 
                        Object.keys(parentSpan.style).length <= 2) {{
                        // Update existing span
                        parentSpan.style.fontFamily = currentFont;
                        parentSpan.style.fontSize = currentFontSize + 'pt';
                        // Ensure cursor stays in place
                        range.setStartAfter(parentSpan.lastChild || parentSpan);
                        range.setEndAfter(parentSpan.lastChild || parentSpan);
                    }} else {{
                        // Insert new span for typing
                        let span = document.createElement('span');
                        span.style.fontFamily = currentFont;
                        span.style.fontSize = currentFontSize + 'pt';
                        span.innerHTML = '\\u200B';
                    ...
                    // Selection exists: Flatten existing spans and apply new style
                    removeFontSpansInRange(range);
                    let wrapper = document.createElement('span');
                    wrapper.style.fontFamily = currentFont;
                    wrapper.style.fontSize = currentFontSize + 'pt';
    ...
    def update_formatting_ui(self, state=None):
        if state:
            ...
            self.size_dropdown.handler_unblock(self.size_dropdown_handler)
################################################################################
###################### 1st size then 2nd onwareds same above
    def apply_font_and_size_to_typing(self):
        script = f"""
            (function() {{
                // Helper function to remove existing font spans within a range (for selection)
                function removeFontSpansInRange(range) {{
                    let walker = document.createTreeWalker(
                        range.commonAncestorContainer,
                        NodeFilter.SHOW_ELEMENT,
                ...
                if (range.collapsed) {{
                    ...
                    // If cursor is at the end of a font span, move outside it
                    if (parent.tagName === 'SPAN' && 
                        ...
                        // Insert a new span after the current one
                        ...
                        range.setEnd(newSpan.firstChild, 1);
                               ...
                               (parent.style.fontFamily || parent.style.fontSize) && 
                               Object.keys(parent.style).length <= 2) {{
                        // Cursor inside a span but not at the end, split and insert new span
                        let newSpan = document.createElement('span');
                        newSpan.style.fontFamily = currentFont;
                        newSpan.style.fontSize = currentFontSize + 'pt';
                        newSpan.innerHTML = '\\u200B';
                        ...
                        // No span or not inside one, create a new span
                        let span = document.createElement('span');
                        span.style.fontFamily = currentFont;
                        span.style.fontSize = currentFontSize + 'pt';
                        span.innerHTML = '\\u200B';
        ...
        """
        self.exec_js(script)
############### above works with typing but not selection, let try selection down:
    def on_size_dropdown_changed(self, dropdown):
        self.current_font_size = dropdown.get_model().get_string(dropdown.get_selected())
        print(f"Dropdown changed to: {self.current_font_size}")
        self.apply_font_and_size_to_typing()
    def apply_font_and_size_to_typing(self):
        print(f"Applying font size: {self.current_font_size} for selection")
        script = f"""
            (function() {{
                let currentFont = '{self.current_font}';
                let currentFontSize = '{self.current_font_size}';
                let sel = window.getSelection();
                if (!sel.rangeCount) {{
                    return 'No selection range found';
                }}
                let range = sel.getRangeAt(0);
                ...
                if (range.collapsed) {{
                    ...
                    sel.removeAllRanges();
                    sel.addRange(range);
                    return 'Collapsed range styled at ' + currentFontSize + 'pt';
                }} else {{
                    ...
                    if (startContainer.nodeType === Node.TEXT_NODE && endContainer.nodeType === Node.TEXT_NODE) {{
                        ...
                        let endText = endContainer.textContent;
                        let startOffset = range.startOffset;
                        let endOffset = range.endOffset;
                        
                        let selectedText = (startContainer === endContainer) 
                            ? startText.substring(startOffset, endOffset)
                            : startText.substring(startOffset) + endText.substring(0, endOffset);
                        ...
                        sel.addRange(range);
                        return 'Styled selection "' + selectedText + '" at ' + currentFontSize + 'pt';
                    }} else {{
                        ...
                        sel.removeAllRanges();
                        range.setStart(wrapper.firstChild || wrapper, 0);
                        range.setEnd(wrapper.lastChild || wrapper, wrapper.textContent.length);
                        sel.addRange(range);
                        return 'Styled complex selection at ' + currentFontSize + 'pt';
                    }}
                }}
            }})();
        """

        def js_callback(result, error, user_data):
            if error:
                print(f"JavaScript error: {error}")
            else:
                print(f"JavaScript result: {result}")
            print(f"Current DOM: {self.webview.evaluate_javascript('document.body.innerHTML', -1, None, None, None, None, None)}")

        self.webview.evaluate_javascript(script, -1, None, None, None, js_callback, None)
        
############### The above works with both selection and typing.
    def apply_font_and_size_to_typing(self):
        script = f"""
            (function() {{
                let currentFont = '{self.current_font}';
        ...
        """
        self.exec_js(script)  # Simplified back to exec_js since it works
