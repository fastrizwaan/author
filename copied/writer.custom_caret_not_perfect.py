...
##########################
                    ...
                    const updateCaret = function() {
                        ...
                        if (sel.rangeCount > 0) {
                            ...
                            if (rects.length > 0) {
                                ...
                                let caretHeight = fontSizePx * 1.1; // 30% bigger for size + descenders
    ...
    def apply_font_and_size_to_typing(self):
        ...
        """
        self.exec_js(script)
############## good above, but font size fail won't reflect
    def on_webview_load(self, webview, load_event):
        if load_event == WebKit.LoadEvent.FINISHED:
            self.webview.evaluate_javascript("""
                (function() {
                    ...
                    const updateCaret = function() {
                        ...
                        const state = {
                            ...
                            formatBlock: document.queryCommandValue('formatBlock') || 'p',
                            fontName: document.queryCommandValue('fontName') || 'Serif',
                            fontSize: parseFloat(fontSize).toString(), // Send actual font size in pt
                            insertUnorderedList: document.queryCommandState('insertUnorderedList'),
                            insertOrderedList: document.queryCommandState('insertOrderedList'),
                            justifyLeft: document.queryCommandState('justifyLeft'),
                            justifyCenter: document.queryCommandState('justifyCenter'),
                        ...
                        if (sel.rangeCount > 0) {
                            ...
                            if (rects.length > 0) {
                                let rect = rects[0];
                                let fontSizePx = parseFloat(fontSize); // Convert to number (e.g., "20pt" -> 20)
                                let caretHeight = fontSizePx * 1.05; // Reduced from 1.1 to 1.05
                                let offset = caretHeight / 2; // Half the caret height for centering
                                customCaret.style.height = caretHeight + 'pt';
                                customCaret.style.left = (rect.left + window.scrollX) + 'px';
                                customCaret.style.top = (rect.top + window.scrollY - offset + fontSizePx * 0.5) + 'px'; // Extended for descenders
...
# above one is good with font size, but caret does not appear on empty lines:
##############################################################################
    def on_webview_load(self, webview, load_event):
        if load_event == WebKit.LoadEvent.FINISHED:
            self.webview.evaluate_javascript("""
                (function() {
                    ...
                    const updateCaret = function() {
                        ...
                        if (sel.rangeCount > 0) {
                            ...
                            let caretHeight = fontSizePx * 1.05;
                            let offset = caretHeight / 2;

                            if (rects.length > 0) {
                                let rect = rects[0];
                                customCaret.style.height = caretHeight + 'pt';
                                customCaret.style.left = (rect.left + window.scrollX) + 'px';
                                customCaret.style.top = (rect.top + window.scrollY - offset + fontSizePx * 0.5) + 'px';
                                customCaret.style.display = 'block';
                            } else {
                                // Fallback: Use parent element's position
                                let parentElement = node.nodeType === 3 ? node.parentElement : node;
                                let parentRect = parentElement.getBoundingClientRect();
                                customCaret.style.height = caretHeight + 'pt';
                                customCaret.style.left = (parentRect.left + window.scrollX) + 'px';
                                customCaret.style.top = (parentRect.top + window.scrollY) + 'px'; // Align with top of empty area
                                customCaret.style.display = 'block';
                            }
                    ...
                    // Ensure caret updates after Tab/Enter
                    document.addEventListener('keydown', function(e) {
                        if (e.key === 'Tab' || e.key === 'Enter') {
                            setTimeout(updateCaret, 0); // Update after default action
                        }
                    });

                    document.addEventListener('click', updateCaret);
                    document.addEventListener('paste', notifyChange);
                    document.addEventListener('cut', notifyChange);
                    document.addEventListener('selectionchange', debouncedUpdateCaret);
                    document.addEventListener('keyup', debouncedUpdateCaret);
