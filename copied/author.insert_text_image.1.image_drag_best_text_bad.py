        ...
        if load_event == WebKit.LoadEvent.FINISHED:
            ...
            enhanced_script = """
                (function() {
                    ...
                    // Add resize handles and make elements draggable
                    function makeElementDraggableAndResizable(element) {
                        ...
                        element.addEventListener('mousedown', function(e) {
                            // Ignore if clicking on resize handle or inside textbox content
                            if (e.target === resizeHandle || 
                            (element.classList.contains('textbox') && e.target === element && e.target.isContentEditable)) {
                                return;
                            }
                            ...
                            // Add events to document to handle movement outside element
                        ...
                        function onMouseMove(e) {
                            ...
                            if (isResizing) {
                                ...
                                if (element.classList.contains('textbox') || !element.classList.contains('resizable-image')) {
                                    element.style.height = newHeight + 'px';
                                } else if (element.classList.contains('resizable-image')) {
                                    // For images, maintain aspect ratio if it's an img element
                    ...
                    document.body.addEventListener('contextmenu', function(e) {
                        ...
                        const isImage = e.target.classList.contains('resizable-image');
                        const isResizeHandle = e.target.classList.contains('resize-handle');
                        
                        // If clicked on a textbox, image, or their direct children
                        if (isTextbox || isImage || isResizeHandle || 
                            e.target.closest('.textbox') || e.target.closest('.resizable-image')) {
                            
                            e.preventDefault();
                            
                            // Get the actual element (might be a child)
                            activeElement = isTextbox || isImage ? e.target : 
                                            isResizeHandle ? e.target.parentElement :
                                            e.target.closest('.textbox') || e.target.closest('.resizable-image');
                            ...
                            menu.innerHTML = `
                                ...
                                <div id="menu-toggle-flow">Toggle Text Flow</div>
