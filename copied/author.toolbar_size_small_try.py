...
class EditorWindow(Adw.ApplicationWindow):
    def __init__(self, **kwargs):
        ...
        """
        self.css_provider.load_from_data(print_css.encode())
...
<body>
    <div class="page">
        <div class="content" contenteditable="true">
            <span class="guard" contenteditable="false">&#8203;</span>
            <p><br></p>
        </div>
    </div>
</body>
    ...
    const setupPageProtection = () => {
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Backspace') {
                ...
                const contentDiv = range.startContainer.closest('.content');
                const guard = contentDiv.querySelector('.guard');
                
                // Check if trying to delete before guard
                if (range.startOffset === 0 && 
                    range.startContainer === guard.nextSibling) {
                    e.preventDefault();
                    return;
                }

                // Check if trying to delete guard
                if (range.startContainer.contains(guard) || 
                    range.endContainer.contains(guard)) {
                    e.preventDefault();
                    return;
                }
            }
        ...
        // Mutation observer to maintain guard
        new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                mutation.removedNodes.forEach((node) => {
                    if (node.classList.contains('guard')) {
                        const newGuard = document.createElement('span');
                        newGuard.className = 'guard';
                        newGuard.contentEditable = 'false';
                        newGuard.innerHTML = '&#8203;';
                        mutation.target.insertBefore(newGuard, mutation.target.firstChild);
                    }
                });
            });
        ...
        // Auto-focus content when clicking page
        document.querySelectorAll('.page').forEach(page => {
            page.addEventListener('click', () => {
                page.querySelector('.content').focus();
            });
        });
    };
    
    // Initialize protection on load and after new page creation
    setupPageProtection();
    document.addEventListener('DOMNodeInserted', setupPageProtection);
    ...
    def on_key_pressed(self, controller, keyval, keycode, state):
        ...
        if ctrl and not shift:
            if keyval == Gdk.KEY_n:
                ...
                return True
            elif keyval == Gdk.KEY_Return and ctrl:
                self.exec_js("""
                    const breakElem = document.createElement('div');
                    breakElem.className = 'page-break';
                    document.execCommand('insertHTML', false, breakElem.outerHTML);
                """)
                return True
