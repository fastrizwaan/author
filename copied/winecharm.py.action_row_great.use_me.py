...
############################### 1050 - 1682 ########################################
    ...
    def create_script_row(self, script_key, script_data):
        # Create a row for a given script with play and options buttons.
        ...
        button.add_css_class("rounded-button")
        button.add_css_class("normal-font")

        label_text = str(script_data.get('progname', '')).replace('_', ' ') or script.stem.replace('_', ' ')
        overlay = Gtk.Overlay()
        ...
        if self.icon_view:
            ...
            label2_widget = Gtk.Label(label=label2) if label2 else None
            label3_widget = Gtk.Label(label=label3) if label3 else None
        else:
            icon = self.load_icon(script, 32, 32)
            icon_image = Gtk.Image.new_from_paintable(icon)
            button.set_size_request(400, 36)
            icon_image.set_pixel_size(32)
            hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
            buttons_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
            label = Gtk.Label(label=label_text)
            label.set_xalign(0)
            # Default limit: 40 characters.
            label.set_max_width_chars(40)
            label.set_ellipsize(Pango.EllipsizeMode.END)
            label2_widget = Gtk.Label(label="")
            label3_widget = Gtk.Label(label="")

        hbox.append(icon_image)
        # Enforce default limit on the label.
        label.set_max_width_chars(40)
        label.set_ellipsize(Pango.EllipsizeMode.END)
        hbox.append(label)
        if self.icon_view and label2_widget:
            hbox.append(label2_widget)
        if self.icon_view and label3_widget:
            hbox.append(label3_widget)
        button.set_child(hbox)

        if script.stem in self.new_scripts:
            label.set_markup(f"<b>{label.get_text()}</b>")
            if label2_widget:
                label2_widget.set_markup(f"<b>{label2_widget.get_text()}</b>")
            if label3_widget:
                label3_widget.set_markup(f"<b>{label3_widget.get_text()}</b>")

        buttons_box.set_margin_end(6)

        play_button = Gtk.Button.new_from_icon_name("media-playback-start-symbolic")
        play_button.set_tooltip_text("Play")
        play_button.set_size_request(50, 40 if self.icon_view else -1)
        ...
        # If the method is available (GTK version supports it), use it.
        if hasattr(overlay, 'set_overlay_pass_through'):
            overlay.set_overlay_pass_through(buttons_box, True)
        buttons_box.set_halign(Gtk.Align.END)
        buttons_box.set_valign(Gtk.Align.CENTER)

        # Store UI references including label widgets.
        self.script_ui_data[script_key] = {
            'row': overlay,
            'play_button': play_button,
            'options_button': options_button,
            'labels': (label, label2_widget, label3_widget),
            'highlighted': False,
            'is_running': False,
        ...
        # Helper to set the maximum character limit for labels.
        def set_label_limit(labels, limit):
            for lab in labels:
                if lab:
                    lab.set_max_width_chars(limit)
                    lab.set_ellipsize(Pango.EllipsizeMode.END)

        def on_row_clicked(btn, key=script_key):
            # Reset other rows to default limit (40).
            for sk, data in self.script_ui_data.items():
                if sk != key:
                    data['play_button'].set_visible(False)
                    data['options_button'].set_visible(False)
                    set_label_limit(data['labels'], 40)
            # Toggle current row.
            new_visible = not self.script_ui_data[key]['play_button'].get_visible()
            play_button.set_visible(new_visible)
            options_button.set_visible(new_visible)
            # When overlay buttons appear, limit label to 30 chars; otherwise, use 40.
            set_label_limit((label, label2_widget, label3_widget), 33 if new_visible else 40)
            self.on_script_row_clicked(key)

        button.connect("clicked", on_row_clicked)
            ...
            if ui_state:
                ...
                if is_running:
                    ...
                    if script_key in self.running_processes:
                        self.running_processes.pop(script_key, None)
                    ui_state['is_running'] = False
                    # Reset labels to default limit (40).
                    if 'labels' in ui_state:
                        for lab in ui_state['labels']:
                            if lab:
                                lab.set_max_width_chars(40)
                                lab.set_ellipsize(Pango.EllipsizeMode.END)
                    if row and not ui_state.get('is_clicked_row', False):
                        self.update_row_highlight(row, False)
                    if play_button:
                        self.set_play_stop_button_state(play_button, False)
                        play_button.set_tooltip_text("Play")
...
###################### 0.95
    def create_script_row(self, script_key, script_data):
        # Create a row using adw.ActionRow with play and options buttons.
        script = Path(script_data['script_path']).expanduser()
        ...
        action_row.add_css_class("rounded-button")
        action_row.add_css_class("normal-font")

        # Full title text.
        full_title = (str(script_data.get('progname', '')) or script.stem).replace('_', ' ')

        # Helper to truncate text.
        def truncate_text(text, limit):
            return text if len(text) <= limit else text[:limit] + "…"

        # Set default title (40 characters).
        action_row.set_title(truncate_text(full_title, 40))
        ...
        if self.icon_view:
            ...
            icon_image = Gtk.Image.new_from_paintable(icon)
            icon_image.set_pixel_size(32)
        action_row.add_prefix(icon_image)

        # Create suffix container with play and options buttons.
        suffix_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        play_button = Gtk.Button.new_from_icon_name("media-playback-start-symbolic")
        play_button.set_tooltip_text("Play")
        play_button.set_size_request(50, 40 if self.icon_view else -1)
        play_button.hide()  # initially hidden
        suffix_box.append(play_button)
        options_button = Gtk.Button.new_from_icon_name("emblem-system-symbolic")
        options_button.set_tooltip_text("Options")
        options_button.hide()  # initially hidden
        suffix_box.append(options_button)
        ...
        def on_row_activated(row, key=script_key):
            # Reset other rows.
            for sk, data in self.script_ui_data.items():
                if sk != key:
                    data['play_button'].hide()
                    data['options_button'].hide()
                    data['row'].set_title(truncate_text(data['full_title'], 40))
            # Toggle current row's buttons.
            new_visible = not self.script_ui_data[key]['play_button'].get_visible()
            if new_visible:
                play_button.show()
                options_button.show()
                action_row.set_title(truncate_text(full_title, 30))
            else:
                play_button.hide()
                options_button.hide()
                action_row.set_title(truncate_text(full_title, 40))
        ...
        action_row.connect("activated", on_row_activated)
        play_button.connect("clicked", lambda btn: self.toggle_play_stop(script_key, play_button, action_row))
        options_button.connect("clicked", lambda btn: self.show_options_for_script(self.script_ui_data[script_key], action_row, script_key))
            ...
            if ui_state:
                ...
                if is_running:
                    ...
                    if script_key in self.running_processes:
                        self.running_processes.pop(script_key, None)
                    ui_state['is_running'] = False
                    # Reset title to default limit (40).
                    row.set_title(truncate_text(ui_state['full_title'], 40))
                    if row and not ui_state.get('is_clicked_row', False):
                        self.update_row_highlight(row, False)
                    if play_button:
                        self.set_play_stop_button_state(play_button, False)
    ...
    def create_script_row(self, script_key, script_data):
        ...
        # Create an ActionRow with title and activatable property.
        row = Adw.ActionRow(
            title=str(script_data.get('progname', script.stem)).replace('_', ' '),
            activatable=True
        )
        ...
        # Create a container for the suffix (overlay) buttons.
        ...
        # Initially hide the overlay buttons.
        play_button.hide()
        options_button.hide()
        ...
        # Connect signals.
        row.connect("activated", lambda *_: self.on_script_row_clicked(script_key))
    ...
    def on_script_row_clicked(self, script_key):
        ...
        # Deselect all other rows and hide their overlay buttons.
        for key, data in self.script_ui_data.items():
            if key != script_key:
                data['row'].remove_css_class("selected")
                self.hide_buttons(data['play_button'], data['options_button'])

        # Toggle the selected state of the current row.
        if current_row.has_css_class("selected"):
            current_row.remove_css_class("selected")
            self.hide_buttons(current_data['play_button'], current_data['options_button'])
        else:
            current_row.add_css_class("selected")
            self.show_buttons(current_data['play_button'], current_data['options_button'])

        # Maintain the "running" highlight if applicable.
        if current_data.get('is_running'):
            current_row.add_css_class("running")
        else:
            current_row.remove_css_class("running")
            ...
            if ui_state:
                ...
                if is_running:
                    ...
                    if script_key in self.running_processes:
                        self.running_processes.pop(script_key, None)
                    ui_state['is_running'] = False
                    # No text truncation; leave the title as is.
                    if row and not ui_state.get('is_clicked_row', False):
                        self.update_row_highlight(row, False)
                    if play_button:
                        self.set_play_stop_button_state(play_button, False)
    ...
    def create_script_row(self, script_key, script_data):
        ...
        # Create an ActionRow with a title and make it activatable.
        row = Adw.ActionRow(
            title=str(script_data.get('progname', script.stem)).replace('_', ' '),
            activatable=True
        )
        ...
        # Add a prefix icon.
        icon = self.load_icon(script, 36, 36)
        if icon:
            icon_image = Gtk.Image.new_from_paintable(icon)
            icon_image.set_pixel_size(36)
            row.add_prefix(icon_image)

        # Create a container (box) for the suffix buttons.
        button_box = Gtk.Box(spacing=6, valign=Gtk.Align.CENTER)
        ...
        # Use a click gesture to toggle the overlay buttons.
    ...
    def toggle_overlay_buttons(self, script_key):
        # First, hide overlay buttons on all other rows.
        for key, ui in self.script_ui_data.items():
            if key != script_key:
                ui['play_button'].hide()
                ui['options_button'].hide()
        # Then toggle the overlay buttons on the clicked row.
        ui = self.script_ui_data.get(script_key)
        if not ui:
            return
        pb = ui['play_button']
...
########
    def toggle_overlay_buttons(self, script_key):
        # First, hide overlay buttons on all other rows and remove the "blue" CSS class.
        for key, ui in self.script_ui_data.items():
            if key != script_key:
                ui['play_button'].hide()
                ui['options_button'].hide()
...
##############3 icon view
    def create_script_row(self, script_key, script_data):
        script = Path(script_data['script_path']).expanduser()
        if self.icon_view:
            # ICON VIEW: Create a horizontal container with a vertical box for icon+label and a vertical box for overlay buttons.
            container = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
            
            # Left side: vertical box with icon and label.
            left_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
            icon = self.load_icon(script, 64, 64)
            if icon:
                icon_image = Gtk.Image.new_from_paintable(icon)
                icon_image.set_pixel_size(64)
                left_box.append(icon_image)
            label = Gtk.Label(label=str(script_data.get('progname', script.stem)).replace('_', ' '))
            left_box.append(label)
            container.append(left_box)
            
            # Right side: vertical box with overlay buttons.
            ...
            # Hide the overlay buttons by default.
            play_button.hide()
            options_button.hide()
            ...
            # Use a click gesture on the container to toggle the overlay buttons.
            click = Gtk.GestureClick()
            click.connect("released", lambda gesture, n, x, y: self.toggle_overlay_buttons(script_key))
    ...
    def toggle_overlay_buttons(self, script_key):
        # Hide overlay buttons on all other rows.
        for key, ui in self.script_ui_data.items():
            if key != script_key:
                ui['play_button'].hide()
                ui['options_button'].hide()
        # Toggle overlay buttons for the clicked row.
        ui = self.script_ui_data.get(script_key)
        if not ui:
            return
        pb, ob = ui['play_button'], ui['options_button']
        if pb.get_visible():
            pb.hide()
            ob.hide()
        else:
...
############### multi row wrap
    def create_script_row(self, script_key, script_data):
        ...
        if self.icon_view:
            # ICON VIEW: Build a square-like layout.
            # Main container: horizontal box.
            container = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
            
            # Left side: vertical box with icon and wrapped label.
            left_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
            ...
            # Label: wrap text into up to 3 lines (each ~20 characters).
            label_text = str(script_data.get('progname', script.stem)).replace('_', ' ')
            # Assume self.wrap_text_at_20_chars returns a tuple: (line1, line2, line3)
            line1, line2, line3 = self.wrap_text_at_20_chars(label_text)
            
            label_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
            main_label = Gtk.Label(label=line1)
            main_label.set_wrap(True)
            label_box.append(main_label)
            if line2:
                second_label = Gtk.Label(label=line2)
                second_label.set_wrap(True)
                label_box.append(second_label)
            if line3:
                third_label = Gtk.Label(label=line3)
            ...
            # Right side: vertical box for overlay buttons.
            button_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6)
            play_button = Gtk.Button(icon_name="media-playback-start-symbolic", tooltip_text="Play")
            play_button.add_css_class("flat")
            options_button = Gtk.Button(icon_name="emblem-system-symbolic", tooltip_text="Options")
            ...
            # Hide overlay buttons by default.
            play_button.hide()
            options_button.hide()
            ...
            # Use a click gesture to toggle overlay buttons.
            click = Gtk.GestureClick()
            click.connect("released", lambda gesture, n, x, y: self.toggle_overlay_buttons(script_key))
            container.add_controller(click)
    ...
    def toggle_overlay_buttons(self, script_key):
        ...
        for key, ui in self.script_ui_data.items():
            if key != script_key:
                ui['play_button'].hide()
                ui['options_button'].hide()
        # Toggle overlay buttons on the clicked row.
        ui = self.script_ui_data.get(script_key)
        if not ui:
            return
        pb = ui['play_button']
...
############## alignment
    def create_script_row(self, script_key, script_data):
        ...
        if self.icon_view:
            ...
            # Left side: vertical box for icon and wrapped label.
            left_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
            ...
            # Label: wrap text into up to 3 lines (~20 chars each).
            label_text = str(script_data.get('progname', script.stem)).replace('_', ' ')
            # Assume wrap_text_at_20_chars returns a tuple: (line1, line2, line3)
            line1, line2, line3 = self.wrap_text_at_20_chars(label_text)
            ...
            # Right side: vertical box for overlay buttons.
            button_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6)
            button_box.set_valign(Gtk.Align.CENTER)  # Vertically center the buttons
            play_button = Gtk.Button(icon_name="media-playback-start-symbolic", tooltip_text="Play")
            play_button.add_css_class("flat")
            options_button = Gtk.Button(icon_name="emblem-system-symbolic", tooltip_text="Options")
            options_button.add_css_class("flat")
            ...
            # Use a click gesture on the container to toggle overlay buttons.
            click = Gtk.GestureClick()
            click.connect("released", lambda gesture, n, x, y: self.toggle_overlay_buttons(script_key))
            container.add_controller(click)
    ...
    def create_script_row(self, script_key, script_data):
        ...
        if self.icon_view:
            ...
            # Add a click gesture to toggle overlay buttons.
            click = Gtk.GestureClick()
            click.connect("released", lambda gesture, n, x, y: self.toggle_overlay_buttons(script_key))
            container.add_controller(click)
...
####################
    def create_script_row(self, script_key, script_data):
        ...
        if self.icon_view:
            # ICON VIEW: Build a square-like layout using the full FlowBoxChild area.
            container = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
            ...
            # LIST VIEW: Use an Adw.ActionRow filling the FlowBoxChild area.
            row = Adw.ActionRow(
                title=str(script_data.get('progname', script.stem)).replace('_', ' '),
                activatable=True
            )
...
############### / great above
    def create_script_row(self, script_key, script_data):
        ...
        if self.icon_view:
            # ICON VIEW: Build a square-like layout that fills the FlowBoxChild area.
            container = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
            container.set_hexpand(True)
            container.set_vexpand(True)
            container.set_valign(Gtk.Align.FILL)
            ...
            # Instead of hiding the buttons (which causes layout shifts), set their opacity to 0 and make them insensitive.
            ...
            # Add a click gesture to toggle overlay buttons without affecting layout.
            click = Gtk.GestureClick()
            click.connect("released", lambda gesture, n, x, y: self.toggle_overlay_buttons(script_key))
            container.add_controller(click)
            ...
            # LIST VIEW: Use an Adw.ActionRow that fills the FlowBoxChild area.
            row = Adw.ActionRow(
                title=str(script_data.get('progname', script.stem)).replace('_', ' '),
                activatable=True
            )
            ...
            # Set overlay buttons invisible by setting their opacity.
            play_button.set_opacity(0)
            options_button.set_opacity(0)
            play_button.set_sensitive(False)
            options_button.set_sensitive(False)
    ...
    def toggle_overlay_buttons(self, script_key):
        # Hide overlay buttons (set opacity to 0 and insensitive) and remove blue CSS from all other rows.
        for key, ui in self.script_ui_data.items():
            if key != script_key:
...
#### /Above very very good.
    ...
    def create_script_row(self, script_key, script_data):
        ...
        if self.icon_view:
            ...
            # The layout: a vertical box with a top horizontal box and a bottom label.
            ...
            options_button = Gtk.Button(icon_name="emblem-system-symbolic", tooltip_text="Options")
            options_button.add_css_class("flat")
            options_button.set_size_request(70, 70)
            ...
            if icon:
                ...
                icon_image = Gtk.Image()  # empty image placeholder
            # Play button (on the right).
            play_button = Gtk.Button(icon_name="media-playback-start-symbolic", tooltip_text="Play")
            play_button.add_css_class("flat")
            play_button.set_size_request(70, 70)
            ...
            # Bottom: Label area.
            label_text = str(script_data.get('progname', script.stem)).replace('_', ' ')
            # Assume wrap_text_at_20_chars returns up to 3 lines.
            line1, line2, line3 = self.wrap_text_at_20_chars(label_text)
            ...
            # Initially hide the overlay buttons by setting their opacity to 0 and disabling them.
            options_button.set_opacity(0)
            play_button.set_opacity(0)
            options_button.set_sensitive(False)
            play_button.set_sensitive(False)
...
#####################3 process ended button show again?
    def process_ended(self, script_key):
        self.print_method_name()
        print(f"--> I'm called by {script_key}")
        ui_state = self.script_ui_data.get(script_key)
    ...
    def create_script_row(self, script_key, script_data):
        ...
        if self.icon_view:
            ...
            if icon:
                ...
                icon_image = Gtk.Image()  # placeholder
            ...
            # Bottom: Label area.
            label_text = str(script_data.get('progname', script.stem)).replace('_', ' ')
            # Assume wrap_text_at_24_chars returns a tuple: (line1, line2, line3)
            ...
            # Initially hide overlay buttons by setting opacity to 0 and making them insensitive.
            play_button.set_opacity(0)
            options_button.set_opacity(0)
            play_button.set_sensitive(False)
            options_button.set_sensitive(False)
            ...
            row.add_css_class('activatable')
            row.set_use_markup(True)
            icon = self.load_icon(script, 48, 48)
            if icon:
                icon_image = Gtk.Image.new_from_paintable(icon)
