...
<body><p></p></body>
        ...
        # Font size dropdown - using proper point sizes instead of WebKit's 1-6 scale
        self.font_sizes = [str(size) for size in [6, 8, 9, 10, 11, 12, 14, 16, 18, 20, 22, 24, 26, 28, 36, 48, 72, 96]]
        size_store = Gtk.StringList(strings=self.font_sizes)
        self.size_dropdown = Gtk.DropDown(model=size_store)
        default_size_index = self.font_sizes.index("12") if "12" in self.font_sizes else 5  # Default to 12pt
    ...
    def on_webview_load(self, webview, load_event):
        if load_event == WebKit.LoadEvent.FINISHED:
            self.webview.evaluate_javascript("""
                (function() {
                    // Set up initial content and selection
                    ...
                    // Initialize current font size storage
                    document.body.setAttribute('data-current-font-size', '12pt');
                    
                    // Handle text insertion with correct font size
                    document.addEventListener('keydown', function(event) {
                        // When typing begins, apply the currently selected font size to new text
                        if (event.key.length === 1 && !event.ctrlKey && !event.metaKey && !event.altKey) {
                            let selection = window.getSelection();
                            if (selection.rangeCount > 0 && selection.getRangeAt(0).collapsed) {
                                let fontSize = document.body.getAttribute('data-current-font-size');
                                if (fontSize) {
                                    // Only proceed if we're not already in a properly sized element
                                    let node = selection.anchorNode;
                                    let isProperlyFormatted = false;
                                    
                                    // Check if we're already in a properly sized element
                                    if (node.nodeType === Node.TEXT_NODE && node.parentNode) {
                                        let computedStyle = window.getComputedStyle(node.parentNode);
                                        isProperlyFormatted = (computedStyle.fontSize === fontSize);
                                    }
                                    
                                    if (!isProperlyFormatted) {
                                        // Create a properly sized span only when actually typing
                    ...
                    // Clean up empty spans periodically
                    const cleanupEmptySpans = debounce(function() {
                        const emptySpans = document.querySelectorAll('span:empty:not(:focus)');
                        for (let span of emptySpans) {
                            // Only remove if it's really empty (no child nodes)
                    ...
                    // Selection change tracking with enhanced font size handling
                    const notifySelectionChange = debounce(function() {
                        ...
                        if (sel.rangeCount > 0) {
                            ...
                            // Store the current font size for future insertions
                            if (fontSize) {
                                if (fontSize.endsWith('px')) {
                                    let fontSizePx = parseFloat(fontSize);
                                    let fontSizePt = Math.round(fontSizePx / 1.333);
                                    fontSize = fontSizePt + 'pt';
                                }
                                document.body.setAttribute('data-current-font-size', fontSize);
                            }
    ...
    def update_formatting_ui(self, state=None):
        if state:
            ...
            closest_size = min(self.font_sizes, key=lambda x: abs(float(x) - float(font_size_pt)))
            selected_size_index = self.font_sizes.index(closest_size)
            self.current_font_size = closest_size
    ...
    def on_font_family_changed(self, dropdown, *args):
        if item := dropdown.get_selected_item():
            ...
            # Use a more reliable way to apply font family
            js_command = f"""
            (function() {{
                let selection = window.getSelection();
                if (selection.rangeCount > 0) {{
                    ...
                    if (range.collapsed) {{
                        // Handle cursor with no selection - apply to parent element or create span
                        let node = range.startContainer;
                        if (node.nodeType === Node.TEXT_NODE) {{
                            node = node.parentNode;
                        }}
                        node.style.fontFamily = {json.dumps(font_family)};
                    }} else {{
                        // Handle text selection
                        document.execCommand('fontName', false, {json.dumps(font_family)});
                        
                        // Clean up excessive font tags (optional cleanup)
                        setTimeout(function() {{
                            let fontTags = document.querySelectorAll('font[face]');
                            fontTags.forEach(font => {{
                                if (font.getAttribute('face') === {json.dumps(font_family)} && 
                                    font.attributes.length === 1 && 
                                    font.childNodes.length === 1 && 
                                    font.childNodes[0].nodeType === Node.TEXT_NODE) {{
                                    
                                    let span = document.createElement('span');
                                    span.style.fontFamily = {json.dumps(font_family)};
                                    span.textContent = font.textContent;
            ...
            """
            self.exec_js(js_command)
            self.update_formatting_ui()

    # And replace the on_font_size_changed method:
    def on_font_size_changed(self, dropdown, *args):
        if item := dropdown.get_selected_item():
            font_size = item.get_string()
            self.current_font_size = font_size
            
            # Improved JavaScript to avoid creating empty spans
            js_command = f"""
            (function() {{
                let selection = window.getSelection();
                if (selection.rangeCount > 0) {{
                    ...
                    // Store the font size for future insertions when cursor is just blinking
                    document.body.setAttribute('data-current-font-size', '{font_size}pt');
                    
                    if (range.collapsed) {{
                        // For cursor only (no selection), just remember the font size
                        // but don't create any elements
                        return;
                    }} else if (!range.toString().trim()) {{
                        // Empty selection, don't do anything
                        return;
                    }} else {{
                        // We have actual text selected, apply the font size
                        let originalStyleWithCSS = document.queryCommandState('styleWithCSS');
                        document.execCommand('styleWithCSS', false, true);
                        
                        // Use execCommand for better compatibility
                        document.execCommand('fontSize', false, '7');  // Use a dummy size
                        
                        // Find the font elements that were just created and replace with proper pt size
                        let fontElements = [];
                        let walker = document.createTreeWalker(
                            range.commonAncestorContainer,
                            NodeFilter.SHOW_ELEMENT,
                            {{ acceptNode: node => node.tagName === 'FONT' && node.size === '7' ? 
                                NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_SKIP }}
                        );
                        
                        while(walker.nextNode()) {{
                            fontElements.push(walker.currentNode);
                        }}
                        
                        for (let font of fontElements) {{
                            let span = document.createElement('span');
                            span.style.fontSize = '{font_size}pt';
                        ...
                        // Restore original styleWithCSS state
                        document.execCommand('styleWithCSS', false, originalStyleWithCSS);
                    }}
                }}
            }})();
            """
