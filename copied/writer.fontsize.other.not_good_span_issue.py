                ...
                (function() {
                    ...
                    const notifySelectionChange = debounce(function() {
                        const state = {
                            ...
                            fontSize: parseFloat(fontSize).toString(),  || '3',
                ...
                (function() {
                    ...
                    const updateSelectionState = function() {
                        ...
                        const state = {
                            ...
                            fontName: document.queryCommandValue('fontName') || 'Serif',
                            fontSize: (function() {
                                let computedStyle = window.getComputedStyle(document.queryCommandEnabled('fontSize') ? 
                                    document.getSelection().anchorNode.parentElement : document.body);
                                let fontSize = computedStyle.fontSize;
                                return fontSize ? parseFloat(fontSize).toString() : '3';
                            })(),
                            insertUnorderedList: document.queryCommandState('insertUnorderedList'),
                            insertOrderedList: document.queryCommandState('insertOrderedList'),
                            justifyLeft: document.queryCommandState('justifyLeft'),
                    ...
                    const debouncedUpdateSelection = debounce(updateSelectionState, 50);
                    ...
                    document.addEventListener('click', updateSelectionState);
                    document.addEventListener('paste', notifyChange);
                    document.addEventListener('cut', notifyChange);
                    document.addEventListener('selectionchange', debouncedUpdateSelection);
                    document.addEventListener('keyup', debouncedUpdateSelection);
                    updateSelectionState();
                })();
            """, -1, None, None, None, None, None)
            GLib.idle_add(self.webview.grab_focus)
###################### the above breaks font size on selection
    def on_webview_load(self, webview, load_event):
        if load_event == WebKit.LoadEvent.FINISHED:
            self.webview.evaluate_javascript("""
                (function() {
