...
#            return None
 ...
 #       script_key = script_data['sha256sum']  # Use sha256sum as the key
    ...
    def delete_runner(self, action=None):
        ...
        dialog.connect("response", self.on_delete_runner_response, runner_combo, combo_runner_paths)
        dialog.present()

    def on_delete_runner_response(self, dialog, response, runner_combo, combo_runner_paths):
        """
        ...
        # Get the selected runner path
        selected_index = runner_combo.get_active()
        if selected_index < 0:
            self.show_info_dialog("Selection Error", "No runner was selected for deletion.")
            return

        selected_runner = combo_runner_paths[selected_index]

        # Attempt to delete the runner and notify the user
        try:
            # Delete the runner directory or file
            if os.path.isdir(selected_runner):
                shutil.rmtree(selected_runner)
            else:
                os.remove(selected_runner)

            # Notify user of successful deletion
            self.show_info_dialog("Deletion Successful", f"Runner '{selected_runner}' deleted successfully.")

        except Exception as e:
            # Handle errors and display error message
            self.show_info_dialog("Deletion Error", f"Error deleting runner '{selected_runner}': {e}")
