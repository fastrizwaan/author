...
<body><p><br></p></body>
        ...
        self.webview = WebKit.WebView(editable=True, user_content_manager=self.content_manager)
        ...
        toolbar_view.set_content(content_box)
         self.content_manager.register_script_message_handler("contentChanged")
        self.content_manager.connect("script-message-received::contentChanged", self.on_content_changed)
        self.webview.load_html(self.initial_html, "file:///")

        # Add JavaScript to detect changes
        script = """
        ...
        user_script = WebKit.UserScript(
            script=script,
            injected_time=WebKit.UserScriptInjectedTime.START,
            inject_frames=WebKit.UserScriptInjectionFrames.ALL_FRAMES
        )
    ...
    def on_content_changed(self, manager, message):
        """Handle content change messages from JavaScript"""
    ...
    def on_input(self, webview):
        """Handle content modification in WebView"""
        self.is_modified = True
        self.update_title()
