...
<body><p><br></p></body>
    ...
    def on_find_clicked(self, btn):
        ...
        dialog = Adw.MessageDialog(
            transient_for=self,
            heading="Find",
            body="Search text in document",
            close_response="cancel",
            modal=False  # Non-modal so user can interact with text
        )
        
        # Create a box to hold the entry and options
        content_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=10)
        
        # Search term entry with a horizontal box for entry and buttons
        search_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=5)
        entry = Gtk.Entry(hexpand=True, placeholder_text="Search term")
        search_box.append(entry)
        
        # Add navigation buttons directly in the dialog
        prev_btn = Gtk.Button(icon_name="go-up")
        prev_btn.set_tooltip_text("Find Previous")
        prev_btn.set_sensitive(False)  # Initially disabled
        search_box.append(prev_btn)
        
        next_btn = Gtk.Button(icon_name="go-down")
        next_btn.set_tooltip_text("Find Next")
        next_btn.set_sensitive(False)  # Initially disabled
        ...
        content_box.append(case_sensitive)
        
        # Status label to show search results
        status_label = Gtk.Label(label="", halign=Gtk.Align.START)
        ...
        def perform_search():
            ...
            is_case_sensitive = case_sensitive.get_active()
            ...
            # JavaScript to find and count occurrences - Non-recursive approach
            flags = "" if is_case_sensitive else "i"
            script = f"""
            (function() {{
                // Clear any previous highlights
                document.querySelectorAll('.wiziwig-highlight').forEach(el => {{
                ...
                let highlights = [];
                let searchTerm = {json.dumps(search_term)};
                let flags = "{flags}";
                ...
                let walker = document.createTreeWalker(
                    ...
                    {{ 
                        acceptNode: function(node) {{
                            ...
                            if (node.parentNode && 
                                /(script|style|textarea|input)/i.test(node.parentNode.tagName)) {{
                ...
                // Process collected nodes (we need to collect them first to avoid DOM modification issues)
                for (let i = 0; i < nodes.length; i++) {{
                    let node = nodes[i];
                    let text = node.nodeValue;
                    let regex = new RegExp(searchTerm, flags + 'g');
                    ...
                    // If we found matches, we need to split this text node
                    if (matches.length > 0) {{
                        ...
                        for (let j = matches.length - 1; j >= 0; j--) {{
                            ...
                            let before = text.substring(0, match.index);
                            let matched = text.substring(match.index, match.index + match.length);
                            let after = text.substring(match.index + match.length);
                            
                            // Create new text nodes and highlight span
            ...
            # Execute search and get match count
            self.webview.evaluate_javascript(script, -1, None, None, None, 
                                             on_search_complete, None)
        
        def on_search_complete(webview, result, user_data):
            try:
                js_result = webview.evaluate_javascript_finish(result)
                if js_result:
                    total = js_result.to_int32()
                    search_state["total_matches"] = total
                    search_state["current_index"] = -1  # Reset current index
                    
                    # Update UI based on results
                    if total > 0:
                        status_label.set_text(f"Found {total} matches")
                        ...
                        status_label.set_text(f"No matches found for '{search_state['term']}'")
                        prev_btn.set_sensitive(False)
                        next_btn.set_sensitive(False)
            except GLib.Error as e:
                print("Search error:", e.message)
                status_label.set_text("Error performing search")
        
        def navigate_to_match(index):
            # Handle wrap-around
            if index >= search_state["total_matches"]:
                index = 0
            elif index < 0:
                index = search_state["total_matches"] - 1
            
            search_state["current_index"] = index
            
            # Update status
            status_label.set_text(f"Match {index + 1} of {search_state['total_matches']}")
            
            # JavaScript to navigate to the specified match
            script = f"""
            (function() {{
                let highlights = document.querySelectorAll('.wiziwig-highlight');
                if ({index} < highlights.length) {{
                    ...
                    current.style.outline = '2px solid #FF8000';
        ...
        def find_next():
            if search_state["total_matches"] > 0:
                navigate_to_match(search_state["current_index"] + 1)
        ...
        entry.connect("changed", lambda w: perform_search())
        entry.connect("activate", lambda w: find_next())
        next_btn.connect("clicked", lambda w: find_next())
        prev_btn.connect("clicked", lambda w: find_previous())
        case_sensitive.connect("toggled", lambda w: perform_search())
        ...
        # Store references to important dialog components
        dialog._find_data = {
            "entry": entry,
            "clear_highlights": clear_highlights
        }
        
        # Store the dialog reference to prevent it from being destroyed
        if not hasattr(self, '_find_dialog'):
            self._find_dialog = None
        
        # If there was a previous dialog, destroy it and clear highlights
        if self._find_dialog:
            if hasattr(self._find_dialog, '_find_data'):
                self._find_dialog._find_data["clear_highlights"]()
