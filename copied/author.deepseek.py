...
class EditorWindow(Adw.ApplicationWindow):
    ...
    def __init__(self, **kwargs):
        ...
        self.current_font = "Sans"  # Persistent font state
        self.current_font_size = "11"  # Persistent font size state (in pt)
...
<head>
    <style>
        body {{ font-family: {self.current_font}; font-size: {self.current_font_size}pt; margin: 20px; line-height: 1.5; }}
...
<body><p></p></body>
        ...
        for icon, handler in [
            ("edit-find", self.on_find_clicked), ("edit-find-replace", self.on_replace_clicked)
        ]:
            btn = Gtk.Button(icon_name=icon)
            btn.add_css_class("flat")
            btn.connect("clicked", handler)
        ...
        self.zoom_dropdown = Gtk.DropDown(model=zoom_store)
        self.zoom_dropdown.set_selected(5)  # 100%
        self.zoom_dropdown.connect("notify::selected", self.on_zoom_changed)
        self.zoom_dropdown.add_css_class("flat")
        view_group.append(self.zoom_dropdown)
    ...
    def set_dropdown_safe(self, dropdown, index):
        """Safely set dropdown index, defaulting to 0 if invalid."""
        if index >= 0 and index < dropdown.get_model().get_n_items():
            dropdown.set_selected(index)
        else:
            dropdown.set_selected(0)  # Default to first item
    ...
    def on_webview_clicked(self, gesture, n_press, x, y):
        ...
        """
        self.webview.evaluate_javascript(script, -1, None, None, None, self.on_click_check_result, None)
    ...
    def adjust_zoom_level(self, delta):
        ...
        model = self.zoom_dropdown.get_model()
        closest_index = 0
        min_diff = float('inf')
        for i in range(model.get_n_items()):
            value = int(model.get_item(i).get_string().rstrip('%'))
            diff = abs(value - zoom_percentage)
            if diff < min_diff:
                min_diff = diff
                closest_index = i
        self.zoom_dropdown.set_selected(closest_index)

    def on_content_changed_js(self, manager, js_result):
        try:
            if js_result and hasattr(js_result, 'get_js_value'):
                js_value = js_result.get_js_value()
                if js_value and js_value.is_string() and js_value.to_string() == 'changed':
    ...
    def on_key_pressed(self, controller, keyval, keycode, state):
        ...
        # Define navigation and editing keys
        navigation_keys = (
            Gdk.KEY_Left, Gdk.KEY_Right, Gdk.KEY_Up, Gdk.KEY_Down,
            Gdk.KEY_Delete, Gdk.KEY_BackSpace, Gdk.KEY_Insert
        )
        
        if keyval in navigation_keys:
            # Allow default behavior (e.g., move cursor, delete text)
            return False

        if ctrl and not shift:
            if keyval in (Gdk.KEY_b, Gdk.KEY_B):
                self.apply_persistent_formatting('bold', not self.is_bold)
                #self.update_formatting_state()
                return True
            elif keyval in (Gdk.KEY_i, Gdk.KEY_I):
                self.apply_persistent_formatting('italic', not self.is_italic)
                self.update_formatting_state()
                return True
            elif keyval in (Gdk.KEY_u, Gdk.KEY_U):
                self.apply_persistent_formatting('underline', not self.is_underline)
                self.update_formatting_state()
                return True
            ...
            if keyval in (Gdk.KEY_s, Gdk.KEY_S):
                ...
                self.apply_persistent_formatting('strikethrough', not self.is_strikethrough)
                self.update_formatting_state()
                return True
            elif keyval in (Gdk.KEY_l, Gdk.KEY_L):
                self.apply_list_formatting('unordered', not self.is_bullet_list)
                self.update_formatting_state()
                return True
            elif keyval in (Gdk.KEY_n, Gdk.KEY_N):
                self.apply_list_formatting('ordered', not self.is_number_list)
                self.update_formatting_state()
                return True

        # Apply font and size to new text on every key press
    ...
    def draw_color_indicator(self, area, cr, width, height, data):
        color = self.current_text_color if area == self.text_color_indicator else self.current_bg_color
    ...
    def on_text_color_dialog_response(self, dialog, result, btn):
        try:
            ...
            print(f"Text color selection error: {e.message}")
    ...
    def on_bg_color_dialog_response(self, dialog, result, btn):
        try:
            ...
            print(f"Background color selection error: {e.message}")
    ...
    def on_dark_mode_toggled(self, btn):
        ...
        """
        btn.set_icon_name("weather-clear-night" if btn.get_active() else "display-brightness")
        self.exec_js(script)
    ...
    def exec_js(self, script, callback=None):
        self.webview.evaluate_javascript(script, -1, None, None, None, callback or self.on_js_executed, None)
    ...
    def update_title(self):
        ...
        title = f"{modified_marker}{os.path.splitext(self.current_file.get_basename())[0] if self.current_file and not self.is_new else f'Document {self.document_number}'} – Author"
    ...
    def generate_default_name(self):
        return f"Document {self.document_number} - {datetime.now().strftime('%Y-%m-%d')}.htm"
    ...
    def on_find_clicked(self, btn):
        dialog = Adw.MessageDialog(transient_for=self, heading="Find", body="Enter search term", close_response="cancel", modal=True)
        entry = Gtk.Entry()
                    ...
                    script = f"""
                        (function() {{
                            ...
                            const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, {{
                                acceptNode: node => (node.parentNode.tagName !== 'SCRIPT' && node.parentNode.tagName !== 'STYLE' && node.nodeValue.trim()) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT
                            }});
                            const textNodes = [];
                            let node;
                            ...
                            textNodes.forEach(node => {{
                                ...
                                matches.forEach(match => {{
                                    ...
                                    if (start > lastIndex) fragment.appendChild(document.createTextNode(node.nodeValue.slice(lastIndex, start)));
    ...
    def on_replace_clicked(self, btn):
        dialog = Adw.MessageDialog(transient_for=self, heading="Replace", body="Enter search and replacement terms", close_response="cancel", modal=True)
        ...
        def on_response(dialog, response):
            if response == "replace":
                ...
                if search and replacement:
                    script = f"""
                        (function() {{
                            let regex = new RegExp({json.dumps(search)}, 'gi');
                            document.body.innerHTML = document.body.innerHTML.replace(regex, {json.dumps(replacement)});
                        }})();
                    """
                    self.exec_js(script)
            dialog.destroy()
    ...
    def on_italic_toggled(self, btn):
        self.apply_persistent_formatting('italic', btn.get_active())
        self.update_formatting_state()

    def on_underline_toggled(self, btn):
        self.apply_persistent_formatting('underline', btn.get_active())
        self.update_formatting_state()

    def on_strikethrough_toggled(self, btn):
        self.apply_persistent_formatting('strikethrough', btn.get_active())
        self.update_formatting_state()

    def on_bullet_list_toggled(self, btn):
        self.apply_list_formatting('unordered', btn.get_active())
        self.update_formatting_state()

    def on_number_list_toggled(self, btn):
        self.apply_list_formatting('ordered', btn.get_active())
        self.update_formatting_state()
    ...
    def on_indent_more(self, btn=None):
        self.exec_js("document.execCommand('indent', false, null)")
        self.update_formatting_state()

    def on_indent_less(self, btn=None):
        self.exec_js("document.execCommand('outdent', false, null)")
        self.update_formatting_state()

    def apply_alignment(self, alignment):
        script = f"document.execCommand('justify{alignment.capitalize()}', false, null);"
        self.exec_js(script)
    ...
    def apply_persistent_formatting(self, format_type, enable):
        cmd = {'bold': 'bold', 'italic': 'italic', 'underline': 'underline', 'strikethrough': 'strikethrough'}.get(format_type)
        if cmd:
            script = f"""
                (function() {{
                    let currentState = document.queryCommandState('{cmd}');
                    if ({str(enable).lower()} !== currentState) {{
    ...
    def update_formatting_state(self):
        script = """
            (function() {
                ...
                let block = parent;
                while (block && !/^(P|DIV|H[1-6])$/i.test(block.tagName)) {
                ...
                let states = {
                    ...
                    blockType: block ? block.tagName.toLowerCase() : 'div'
                };
        ...
        try:
            ...
            if js_value and js_value.is_string():
                ...
                if states.get('justifyLeft', False):
                    self.is_align_left, self.is_align_center, self.is_align_right, self.is_align_justify = True, False, False, False
                    self.update_alignment_buttons()
                elif states.get('justifyCenter', False):
                    self.is_align_left, self.is_align_center, self.is_align_right, self.is_align_justify = False, True, False, False
                    self.update_alignment_buttons()
                elif states.get('justifyRight', False):
                    self.is_align_left, self.is_align_center, self.is_align_right, self.is_align_justify = False, False, True, False
                    self.update_alignment_buttons()
                elif states.get('justifyFull', False):
                    self.is_align_left, self.is_align_center, self.is_align_right, self.is_align_justify = False, False, False, True
                    self.update_alignment_buttons()
                if 'fontFamily' in states:
                    font_family = states['fontFamily']
                    model = self.font_dropdown.get_model()
                    font_index = -1
                    for i in range(model.get_n_items()):
                        if model.get_item(i).get_string().lower() == font_family.lower():
                            font_index = i
                            break
                    self.set_dropdown_safe(self.font_dropdown, font_index)
                    self.current_font = model.get_item(self.font_dropdown.get_selected()).get_string()
                if 'fontSize' in states:
                    font_size_px = states['fontSize']
                    font_size_pt = str(round(float(font_size_px.replace('px', '')) * 0.75))
                    model = self.size_dropdown.get_model()
                    size_index = -1
                    for i in range(model.get_n_items()):
                        if model.get_item(i).get_string() == font_size_pt:
                            size_index = i
                            break
                    self.set_dropdown_safe(self.size_dropdown, size_index)
                    self.current_font_size = model.get_item(self.size_dropdown.get_selected()).get_string()
                if 'color' in states:
                    self.current_text_color.parse(states['color'])
                    self.text_color_indicator.queue_draw()
                if 'backgroundColor' in states:
                    self.current_bg_color.parse(states['backgroundColor'])
                    self.bg_color_indicator.queue_draw()
                if 'blockType' in states:
                    block_type = states['blockType']
                    heading_map = {'div': 0, 'h1': 1, 'h2': 2, 'h3': 3, 'h4': 4, 'h5': 5, 'h6': 6}
                    heading_index = heading_map.get(block_type, 0)  # Default to "Normal" (0)
                    self.set_dropdown_safe(self.heading_dropdown, heading_index)
        except Exception as e:
    ...
    def save_html_callback(self, webview, result, file):
        try:
            ...
            print(f"HTML save error: {e.message}")
    ...
    def check_save_before_new(self):
        if self.is_modified:
            dialog = Adw.MessageDialog(transient_for=self, heading="Save changes?", body="Do you want to save changes before starting a new document?", close_response="cancel", modal=True)
            dialog.add_response("cancel", "Cancel")
    ...
    def check_save_before_close(self):
        if self.is_modified:
            dialog = Adw.MessageDialog(transient_for=self, heading="Save changes?", body="Do you want to save changes before closing?", close_response="cancel", modal=True)
            dialog.add_response("cancel", "Cancel")
            dialog.add_response("discard", "Discard")
            dialog.add_response("save", "Save")
            dialog.set_response_appearance("save", Adw.ResponseAppearance.SUGGESTED)
    ...
    def start_new_document(self):
        ...
        self.document_number = EditorWindow.document_counter
        EditorWindow.document_counter += 1
        self.is_bold = self.is_italic = self.is_underline = self.is_strikethrough = self.is_bullet_list = self.is_number_list = False
        self.is_align_left, self.is_align_center, self.is_align_right, self.is_align_justify = True, False, False, False
...
###########################
    def apply_persistent_formatting(self, format_type, enable):
        cmd_map = {
            ...
            'strikethrough': ('strikethrough', 'textDecoration', 'line-through')
        }
        cmd_info = cmd_map.get(format_type)
        if not cmd_info:
            return
        cmd, style_property, style_value = cmd_info

        script = f"""
            (function() {{
                const cmd = '{cmd}';
                const styleProperty = '{style_property}';
                const styleValue = '{style_value}';
                const sel = window.getSelection();
                if (!sel.rangeCount) return;
                const range = sel.getRangeAt(0);
                const isActive = document.queryCommandState(cmd);
                const enable = {str(enable).lower()};
                
                if (range.collapsed) {{
                    // Handle collapsed selection by inserting a styled span
                    let span = document.createElement('span');
                    let newValue = enable ? styleValue : 'normal';
                    
                    // For textDecoration, need to handle existing values
                    if (styleProperty === 'textDecoration') {{
                        let currentDecoration = document.queryCommandValue(cmd) || '';
                        if (enable) {{
                            newValue = currentDecoration.includes(styleValue) ? currentDecoration : currentDecoration + ' ' + styleValue;
                        }} else {{
                            newValue = currentDecoration.replace(styleValue, '').trim();
                        }}
                        if (!newValue) newValue = 'none';
                    }}
                    
                    span.style[styleProperty] = newValue;
                    ...
                    // Ensure the command state is updated
                    if (enable !== isActive) {{
                        document.execCommand(cmd, false, null);
                    }}
                }} else {{
                    // Apply command to selected text
                    document.execCommand(cmd, false, null);
                }}
            }})();
        """
        self.exec_js(script, lambda *_: self.update_formatting_state())
