    ...
    def create_script_row(self, script_key, script_data):
        ...
        if self.icon_view:
            ...
            self.script_ui_data[script_key] = {
                'is_selected': False,
                'is_running': False,
                'icon_row': icon_row_widget,
                'list_row': list_row_widget,
                'icon_play_button': icon_play_button_widget,
                'icon_options_button': icon_options_button_widget,
                'list_play_button': list_play_button_widget,
                'list_options_button': list_options_button_widget
            }
...
###################### 0.95
####################################### NEW loaded icon view:

    def create_script_list(self):
        self.print_method_name()
        """Create UI rows for scripts efficiently, pre-building both icon and list views."""
        self.flowbox.remove_all()
        ...
        # Pre-create rows for both views
        for script_key, script_data in self.script_list.items():
            # Create icon view row
            icon_row = self.create_script_row(script_key, script_data, icon_view=True)
            # Create list view row
            list_row = self.create_script_row(script_key, script_data, icon_view=False)
            
            # Store both rows in script_ui_data
            self.script_ui_data[script_key]['icon_row'] = icon_row
            self.script_ui_data[script_key]['list_row'] = list_row
            
            # Highlight based on running state
            if script_key in self.running_processes:
                self.update_row_highlight(icon_row, True)
                self.update_row_highlight(list_row, True)
                self.script_ui_data[script_key]['highlighted'] = True
                self.script_ui_data[script_key]['is_running'] = True
            else:
                self.update_row_highlight(icon_row, False)
                self.update_row_highlight(list_row, False)
                self.script_ui_data[script_key]['highlighted'] = False
                self.script_ui_data[script_key]['is_running'] = False
        
        # Populate flowbox with the current view
        self.update_flowbox_view()
        
        GLib.idle_add(self.check_running_processes_on_startup)

    def update_flowbox_view(self):
        """Update the flowbox with pre-created rows based on the current view mode."""
        self.flowbox.remove_all()
        for script_key in self.script_list.keys():
            row = (self.script_ui_data[script_key]['icon_row'] if self.icon_view 
                else self.script_ui_data[script_key]['list_row'])
            self.flowbox.append(row)
        
        # Adjust layout based on view mode
        #self.flowbox.set_max_children_per_line(8 if self.icon_view else 1)

    def create_script_row(self, script_key, script_data, icon_view):
        script = Path(str(script_data['script_path'])).expanduser()
        ...
        if icon_view:
            ...
            # Store UI data (only once per script_key)
            if script_key not in self.script_ui_data:
                self.script_ui_data[script_key] = {
            ...
            main_label.set_wrap(True)
            main_label.set_ellipsize(Pango.EllipsizeMode.END)
            main_label.set_halign(Gtk.Align.FILL)
            main_label.set_markup(title_text)
            label_box.append(main_label)
            ...
            play_button = Gtk.Button(icon_name="media-playback-start-symbolic", tooltip_text="Play")
            play_button.set_size_request(60, 34)
            
            options_button = Gtk.Button(icon_name="emblem-system-symbolic", tooltip_text="Options")
            options_button.set_size_request(34, 34)
    ...
    def on_view_toggle_button_clicked(self, button):
        ...
        # Switch the flowbox content to the pre-created rows
        self.update_flowbox_view()
    ...
    def toggle_overlay_buttons(self, script_key):
        ...
        for key, ui in self.script_ui_data.items():
            if key != script_key:
                ...
                ui['play_button'].set_sensitive(False)
                ui['options_button'].set_sensitive(False)
                # Apply to both icon_row and list_row to keep them in sync
                ui['icon_row'].remove_css_class("blue")
                ui['list_row'].remove_css_class("blue")
        ...
        pb = ui['play_button']
        ob = ui['options_button']
        # Select the row based on the current view
        row = ui['icon_row'] if self.icon_view else ui['list_row']
    ...
    def on_script_row_clicked(self, script_key):
        ...
        # Track the previously clicked row and update the `is_clicked_row` state
        for key, data in self.script_ui_data.items():
            if data.get('is_clicked_row', False):
                if key != script_key:
                    data['is_clicked_row'] = False
                    data['icon_row'].remove_css_class("blue")
                    data['list_row'].remove_css_class("blue")
                    self.hide_buttons(data['play_button'], data['options_button'])
                    print(f"Removing 'blue' highlight for previously clicked row with script_key: {key}")

        # Toggle the `is_clicked_row` state for the current row
        current_data['is_clicked_row'] = not current_data.get('is_clicked_row', False)
        print(f"script_key = {script_key} is set to data['is_clicked_row'] = {current_data['is_clicked_row']}")

        # Use the row for the current view
        row = current_data['icon_row'] if self.icon_view else current_data['list_row']
        play_button = current_data['play_button']
        options_button = current_data['options_button']
        is_running = current_data['is_running']
        is_clicked = current_data['is_clicked_row']
...
################################# list view buttons not shwoing
    def create_script_row(self, script_key, script_data, icon_view):
        ...
        if script_key not in self.script_ui_data:
            self.script_ui_data[script_key] = {
                ...
                'is_running': False,
                'script_path': script,
                'is_clicked_row': False  # Initialize this here for consistency
            }
    ...
    def toggle_overlay_buttons(self, script_key):
        ...
        # Toggle overlay buttons for both views
        if pb.get_opacity() > 0:
            pb.set_opacity(0)
            ob.set_opacity(0)
            pb.set_sensitive(False)
            ...
            ui['icon_row'].remove_css_class("blue")
            ui['list_row'].remove_css_class("blue")
            ui['is_clicked_row'] = False
        else:
            pb.set_opacity(1)
            ob.set_opacity(1)
            pb.set_sensitive(True)
            ob.set_sensitive(True)
            ui['icon_row'].add_css_class("blue")
            ui['list_row'].add_css_class("blue")
            ui['is_clicked_row'] = True
    ...
    def update_flowbox_view(self):
        ...
        self.flowbox.remove_all()
        for script_key in self.script_list.keys():
            ui = self.script_ui_data[script_key]
            row = ui['icon_row'] if self.icon_view else ui['list_row']
            self.flowbox.append(row)
            
            # Sync button visibility with current state
            if ui['is_clicked_row'] or ui['is_running']:
    ...
    def on_script_row_clicked(self, script_key):
        ...
        # Unhighlight and hide buttons for previously clicked rows
        for key, data in self.script_ui_data.items():
            if data.get('is_clicked_row', False) and key != script_key:
                data['is_clicked_row'] = False
                data['icon_row'].remove_css_class("blue")
                data['list_row'].remove_css_class("blue")
                data['play_button'].set_opacity(0)
                data['options_button'].set_opacity(0)
                data['play_button'].set_sensitive(False)
                data['options_button'].set_sensitive(False)
                print(f"Removing 'blue' highlight for previously clicked row with script_key: {key}")
        ...
        if is_clicked:
            current_data['icon_row'].remove_css_class("highlight")
            current_data['list_row'].remove_css_class("highlight")
            current_data['icon_row'].add_css_class("blue")
            current_data['list_row'].add_css_class("blue")
            ...
            current_data['icon_row'].remove_css_class("blue")
            current_data['list_row'].remove_css_class("blue")
            play_button.set_opacity(0)
            options_button.set_opacity(0)
            play_button.set_sensitive(False)
            options_button.set_sensitive(False)
        ...
        if is_running:
            self.set_play_stop_button_state(play_button, True)
            current_data['icon_row'].add_css_class("highlighted")
            current_data['list_row'].add_css_class("highlighted")
            print(f"Script {script_key} is running. Setting play button to 'Stop' and adding 'highlighted'.")
        else:
            if not is_clicked:
                self.set_play_stop_button_state(play_button, False)
                current_data['icon_row'].remove_css_class("highlighted")
                current_data['list_row'].remove_css_class("highlighted")
                print(f"Script {script_key} is not running. Setting play button to 'Play' and removing 'highlighted'.")
            if is_clicked and not is_running:
                current_data['icon_row'].add_css_class("blue")
                current_data['list_row'].add_css_class("blue")
...
###################### list view buttons not visible above:
    def update_flowbox_view(self):
        # Clear the current view
        self.flowbox.remove_all()

        # Populate the flowbox with the appropriate rows
        for script_key in self.script_list.keys():
            ui = self.script_ui_data[script_key]
            row = ui['icon_row'] if self.icon_view else ui['list_row']
            self.flowbox.append(row)

            # Sync row highlighting with the clicked state
            if ui.get('is_clicked_row', False):
                row.add_css_class("blue")
            else:
                row.remove_css_class("blue")
    ...
    def toggle_overlay_buttons(self, script_key):
        # Reset all rows and buttons except the clicked one
        for key, ui in self.script_ui_data.items():
            if key != script_key:
                ui['play_button'].set_opacity(0)
                ui['options_button'].set_opacity(0)
        ...
        # Get the UI data for the clicked row
        ui = self.script_ui_data.get(script_key)
        if not ui:
            return

        # Reference the shared buttons and the current view's row
        play_button = ui['play_button']
        options_button = ui['options_button']
        current_row = ui['icon_row'] if self.icon_view else ui['list_row']

        # Toggle the clicked state
        ui['is_clicked_row'] = not ui.get('is_clicked_row', False)

        # Update button visibility and row highlighting
        if ui['is_clicked_row']:
            play_button.set_opacity(1)
            options_button.set_opacity(1)
            play_button.set_sensitive(True)
            options_button.set_sensitive(True)
            current_row.add_css_class("blue")
        else:
            play_button.set_opacity(1)
            options_button.set_opacity(1)
            play_button.set_sensitive(False)
            options_button.set_sensitive(False)
            current_row.remove_css_class("blue")
################ finally?
    def create_script_row(self, script_key, script_data, icon_view):
        script = Path(str(script_data['script_path'])).expanduser()
        ...
        if icon_view:
            ...
            # Initial button states for icon view
            play_button.set_opacity(0)
            options_button.set_opacity(0)
            play_button.set_sensitive(False)
            options_button.set_sensitive(False)
            ...
            if icon:
                ...
                icon_container.set_margin_start(6)
                main_label.set_wrap(True)
                main_label.set_ellipsize(Pango.EllipsizeMode.NONE)
            ...
            label_box.set_hexpand(True)
            main_label.set_max_width_chars(50)
            main_label.set_halign(Gtk.Align.FILL)
            main_label.set_markup(title_text)
            label_box.append(main_label)
            ...
            # Initial button states for list view
            play_button.set_opacity(0)
            options_button.set_opacity(0)
            play_button.set_sensitive(False)
            options_button.set_sensitive(False)

        # Store UI data in self.script_ui_data
        if script_key not in self.script_ui_data:
            self.script_ui_data[script_key] = {
                'is_running': False,
                'script_path': script,
        ...
        if icon_view:
            self.script_ui_data[script_key]['icon_play_button'] = play_button
            self.script_ui_data[script_key]['icon_options_button'] = options_button
            self.script_ui_data[script_key]['icon_row'] = container
        else:
            self.script_ui_data[script_key]['list_play_button'] = play_button
            self.script_ui_data[script_key]['list_options_button'] = options_button
            self.script_ui_data[script_key]['list_row'] = container

        # Add click gesture to toggle buttons
        click = Gtk.GestureClick()
        click.connect("released", lambda gesture, n, x, y: self.toggle_overlay_buttons(script_key))
        container.add_controller(click)
    ...
    def toggle_overlay_buttons(self, script_key):
        ...
        for key, ui in self.script_ui_data.items():
            if key != script_key:
                ui['icon_play_button'].set_opacity(0)
                ui['icon_options_button'].set_opacity(0)
                ui['icon_play_button'].set_sensitive(False)
                ui['icon_options_button'].set_sensitive(False)
                ui['list_play_button'].set_opacity(0)
                ui['list_options_button'].set_opacity(0)
                ui['list_play_button'].set_sensitive(False)
                ui['list_options_button'].set_sensitive(False)
                ui['icon_row'].remove_css_class("blue")
                ui['list_row'].remove_css_class("blue")
                ui['is_clicked_row'] = False
        ...
        if self.icon_view:
            pb = ui['icon_play_button']
            ob = ui['icon_options_button']
            row = ui['icon_row']
        else:
            pb = ui['list_play_button']
            ob = ui['list_options_button']
            row = ui['list_row']
        ...
        if pb.get_opacity() > 0:
            ...
            #pb.set_opacity(1)
            #ob.set_opacity(1)
            ui['icon_play_button'].set_opacity(1)
            ui['icon_options_button'].set_opacity(1)
            ui['list_play_button'].set_opacity(1)
            ui['list_options_button'].set_opacity(1)
            #pb.set_sensitive(True)
            #ob.set_sensitive(True)
            ui['icon_play_button'].set_sensitive(True)
            ui['icon_options_button'].set_sensitive(True)
            ui['list_play_button'].set_sensitive(True)
            ui['list_options_button'].set_sensitive(True)
            #row.add_css_class("blue")
            ui['list_row'].add_css_class("blue")
            ui['icon_row'].add_css_class("blue")
            ui['is_clicked_row'] = True
        ...
        if ui.get("is_running"):
            pb.set_icon_name("media-playback-stop-symbolic")
            ui['icon_play_button'].set_icon_name("media-playback-stop-symbolic")
            ui['list_play_button'].set_icon_name("media-playback-stop-symbolic")
        else:
            pb.set_icon_name("media-playback-start-symbolic")
            ui['icon_play_button'].set_icon_name("media-playback-start-symbolic")
            ui['list_play_button'].set_icon_name("media-playback-start-symbolic")
    ...
    def on_script_row_clicked(self, script_key):
        ...
        for key, data in self.script_ui_data.items():
            if data.get('is_clicked_row', False) and key != script_key:
                ...
                data['icon_row'].remove_css_class("blue")
                data['list_row'].remove_css_class("blue")
                data['icon_play_button'].set_opacity(0)
                data['icon_options_button'].set_opacity(0)
                data['icon_play_button'].set_sensitive(False)
                data['icon_options_button'].set_sensitive(False)
                data['list_play_button'].set_opacity(0)
                data['list_options_button'].set_opacity(0)
                data['list_play_button'].set_sensitive(False)
                data['list_options_button'].set_sensitive(False)
                print(f"Removing 'blue' highlight for previously clicked row with script_key: {key}")
        ...
        if self.icon_view:
            row = current_data['icon_row']
            play_button = current_data['icon_play_button']
            options_button = current_data['icon_options_button']
        else:
            row = current_data['list_row']
            play_button = current_data['list_play_button']
            options_button = current_data['list_options_button']
        ...
        if is_clicked:
            ...
            play_button.set_sensitive(True)
            options_button.set_sensitive(True)
            current_data['list_play_button'].set_opacity(1)
            current_data['icon_play_button'].set_opacity(1)
            #options_button.set_opacity(1)
            current_data['list_play_button'].set_sensitive(True)
            current_data['icon_play_button'].set_sensitive(True)
            # row.add_css_class("blue")
            current_data['icon_row'].add_css_class("blue")
            current_data['list_row'].add_css_class("blue")
            print(f"Highlighting clicked row for script_key: {script_key} with 'blue'")
...
# ######### above fixed but list view is using ellipsed text, and icon view blue css not 
...
#         # Unhighlight and hide buttons for previously clicked rows (except the current one)
#         for key, data in self.script_ui_data.items():
#             if data.get('is_clicked_row', False) and key != script_key:
#                 data['is_clicked_row'] = False
#                 data['icon_row'].remove_css_class("blue")
...
#         # Set the clicked state to True (instead of toggling)
#         current_data['is_clicked_row'] = True
#         print(f"script_key = {script_key} is set to data['is_clicked_row'] = {current_data['is_clicked_row']}")

#         # Select buttons and row based on current view
#         if self.icon_view:
#             row = current_data['icon_row']
#             play_button = current_data['icon_play_button']
#             options_button = current_data['icon_options_button']
...
#         # Since is_clicked is now always True on click, always show buttons and highlight
#         current_data['icon_row'].remove_css_class("highlight")
#         current_data['list_row'].remove_css_class("highlight")
#         row.add_css_class("blue")
#         play_button.set_opacity(1)
...
#         else:
#             self.set_play_stop_button_state(play_button, False)
#             # Only remove "highlighted" if not running, keep "blue" for clicked state
#             if not is_running:
#                 current_data['icon_row'].remove_css_class("highlighted")
#                 current_data['list_row'].remove_css_class("highlighted")
...
#         # Set to clicked state (no toggle off)
#         ui['is_clicked_row'] = True
#         pb.set_opacity(1)
#         ob.set_opacity(1)
#         pb.set_sensitive(True)
...
# ################ fixed?
#     def on_script_row_clicked(self, script_key):
#         """Handle row click, syncing highlight and button states for both views."""
#         self.count = 0
#         self.print_method_name()
#         current_data = self.script_ui_data.get(script_key)
#         if not current_data:
...
#                 data['list_play_button'].set_sensitive(False)
#                 data['list_options_button'].set_sensitive(False)
#                 print(f"Resetting previously clicked row with script_key: {key}")

#         # Set the clicked state for the current row
#         current_data['is_clicked_row'] = True
#         print(f"script_key = {script_key} is_clicked_row = {current_data['is_clicked_row']}")

#         # Apply highlight to both rows
#         current_data['icon_row'].add_css_class("blue")
#         current_data['list_row'].add_css_class("blue")

#         # Update buttons for BOTH views (not just the current one)
#         for play_button, options_button in [
#             (current_data['icon_play_button'], current_data['icon_options_button']),
#             (current_data['list_play_button'], current_data['list_options_button'])
#         ]:
#             play_button.set_opacity(1)
#             options_button.set_opacity(1)
#             play_button.set_sensitive(True)
...
#             is_running = current_data.get('is_running', False)
#             if is_running:
#                 self.set_play_stop_button_state(play_button, True)
...
#     def update_flowbox_view(self):
#         """Update the flowbox with the correct rows for the current view mode."""
#         self.flowbox.remove_all()
#         for script_key in self.script_list.keys():
#             ui = self.script_ui_data[script_key]
#             row = ui['icon_row'] if self.icon_view else ui['list_row']
...
#             # Safeguard: Ensure button states match is_clicked_row and is_running
#             play_button = ui['icon_play_button'] if self.icon_view else ui['list_play_button']
#             options_button = ui['icon_options_button'] if self.icon_view else ui['list_options_button']
            
#             if ui.get('is_clicked_row', False) or ui.get('is_running', False):
#                 play_button.set_opacity(1)
#                 options_button.set_opacity(1)
#                 play_button.set_sensitive(True)
#                 options_button.set_sensitive(True)
#                 row.add_css_class("blue")
#                 if ui.get('is_running', False):
#                     self.set_play_stop_button_state(play_button, True)
#                 else:
#                     self.set_play_stop_button_state(play_button, False)
#             else:
...
# ############## fix 2
#     def update_flowbox_view(self):
#         """Update the flowbox with the correct rows and simulate a click on the selected row."""
#         self.flowbox.remove_all()
#         selected_script_key = None

#         # Populate the flowbox and find the selected row
#         for script_key in self.script_list.keys():
#             ui = self.script_ui_data[script_key]
#             row = ui['icon_row'] if self.icon_view else ui['list_row']
#             self.flowbox.append(row)
...
#             if ui.get('is_clicked_row', False):
#                 selected_script_key = script_key

#         # If there’s a selected row, simulate a click on it in the new view
#         if selected_script_key:
#             self.on_script_row_clicked(selected_script_key)

#     def on_script_row_clicked(self, script_key):
#         """Handle row click, toggling selection and syncing states for both views."""
#         current_data = self.script_ui_data.get(script_key)
#         if not current_data:
#             print(f"No script data found for script_key: {script_key}")
#             return

#         # Toggle the clicked state for the current row
#         current_data['is_clicked_row'] = not current_data.get('is_clicked_row', False)
#         print(f"script_key = {script_key} is_clicked_row = {current_data['is_clicked_row']}")

#         # Update all rows: unselect others if this one is selected, or just update this one
#         for key, data in self.script_ui_data.items():
#             # Handle the clicked row
#             if key == script_key:
#                 row_icon = data['icon_row']
#                 row_list = data['list_row']
#                 play_icon = data['icon_play_button']
#                 play_list = data['list_play_button']
#                 options_icon = data['icon_options_button']
#                 options_list = data['list_options_button']

#                 if data['is_clicked_row']:
#                     row_icon.add_css_class("blue")
#                     row_list.add_css_class("blue")
#                     play_icon.set_opacity(1)
#                     play_list.set_opacity(1)
#                     options_icon.set_opacity(1)
#                     options_list.set_opacity(1)
#                     play_icon.set_sensitive(True)
#                     play_list.set_sensitive(True)
#                     options_icon.set_sensitive(True)
#                     options_list.set_sensitive(True)
#                 else:
#                     row_icon.remove_css_class("blue")
#                     row_list.remove_css_class("blue")
#                     play_icon.set_opacity(0)
#                     play_list.set_opacity(0)
#                     options_icon.set_opacity(0)
#                     options_list.set_opacity(0)
#                     play_icon.set_sensitive(False)
#                     play_list.set_sensitive(False)
#                     options_icon.set_sensitive(False)
#                     options_list.set_sensitive(False)

#                 # Handle running state
#                 is_running = data.get('is_running', False)
#                 self.set_play_stop_button_state(play_icon, is_running)
#                 self.set_play_stop_button_state(play_list, is_running)

#             # Unselect other rows if this one was selected
#             elif current_data['is_clicked_row'] and data.get('is_clicked_row', False):
#                 data['is_clicked_row'] = False
#                 data['icon_row'].remove_css_class("blue")
#                 data['list_row'].remove_css_class("blue")
#                 data['icon_play_button'].set_opacity(0)
...
#                 data['list_play_button'].set_sensitive(False)
#                 data['list_options_button'].set_sensitive(False)
#                 print(f"Resetting previously selected row with script_key: {key}")
...
#         current_data = self.script_ui_data.get(script_key)
#         if not current_data:
#             print(f"No data found for script_key: {script_key}")
#             return

#         # Toggle the selection state
#         current_data['is_clicked_row'] = not current_data.get('is_clicked_row', True)
#         print(f"Clicked {script_key}, is_clicked_row = {current_data['is_clicked_row']}")
...
#         # Update the clicked row’s appearance in both views
#         if current_data['is_clicked_row']:
#             current_data['icon_row'].add_css_class("blue")
#             current_data['list_row'].add_css_class("blue")
#             current_data['icon_play_button'].set_opacity(1)
#             current_data['icon_options_button'].set_opacity(1)
#             current_data['icon_play_button'].set_sensitive(True)
#             current_data['icon_options_button'].set_sensitive(True)
#             current_data['list_play_button'].set_opacity(1)
#             current_data['list_options_button'].set_opacity(1)
#             current_data['list_play_button'].set_sensitive(True)
#             current_data['list_options_button'].set_sensitive(True)
#         else:
#             current_data['icon_row'].remove_css_class("blue")
#             current_data['list_row'].remove_css_class("blue")
#             current_data['icon_play_button'].set_opacity(0)
#             current_data['icon_options_button'].set_opacity(0)
#             current_data['icon_play_button'].set_sensitive(False)
#             current_data['icon_options_button'].set_sensitive(False)
#             current_data['list_play_button'].set_opacity(0)
#             current_data['list_options_button'].set_opacity(0)
#             current_data['list_play_button'].set_sensitive(False)
#             current_data['list_options_button'].set_sensitive(False)
...
# ################# fix 4?
#     def on_view_toggle_button_clicked(self, button):
#         self.icon_view = not self.icon_view  # Toggle between True and False
#         self.update_flowbox_view()  # Refresh the display
#     def update_flowbox_view(self):
#         self.flowbox.remove_all()
#         for script_key, ui in self.script_ui_data.items():
#             row = ui['icon_row'] if self.icon_view else ui['list_row']
#             self.flowbox.append(row)
#             self.update_running_state(script_key, ui['is_running'])
        
#     def update_running_state(self, script_key, is_running):
#         ui = self.script_ui_data[script_key]
#         ui['is_running'] = is_running
#         if is_running:
#             ui['icon_row'].add_css_class("highlighted")
#             ui['list_row'].add_css_class("highlighted")
#         else:
#             ui['icon_row'].remove_css_class("highlighted")
#             ui['list_row'].remove_css_class("highlighted")
#         # Update play button icon (e.g., play/stop)
#         self.set_play_stop_button_state(ui['icon_play_button'], is_running)
#         self.set_play_stop_button_state(ui['list_play_button'], is_running)

#     def on_script_row_clicked(self, row, script_key):
#         current_data = self.script_ui_data[script_key]
#         # Toggle selection state
#         current_data['is_selected'] = not current_data['is_selected']

#         if current_data['is_selected']:
#             # Select the row: highlight and show buttons
#             current_data['icon_row'].add_css_class("blue")
#             current_data['list_row'].add_css_class("blue")
#             current_data['icon_play_button'].set_visible(True)
#             current_data['icon_options_button'].set_visible(True)
#             current_data['icon_play_button'].set_sensitive(True)
#             current_data['icon_options_button'].set_sensitive(True)
#             current_data['list_play_button'].set_visible(True)
#             current_data['list_options_button'].set_visible(True)
#             current_data['list_play_button'].set_sensitive(True)
#             current_data['list_options_button'].set_sensitive(True)
#         else:
#             # Unselect the row: remove highlight and hide buttons
#             current_data['icon_row'].remove_css_class("blue")
#             current_data['list_row'].remove_css_class("blue")
#             current_data['icon_play_button'].set_visible(False)
#             current_data['icon_options_button'].set_visible(False)
#             current_data['list_play_button'].set_visible(False)
#             current_data['list_options_button'].set_visible(False)



# ##############
#     def oxn_script_row_clicked(self, script_key):
...
#         current_data['is_selected'] = True        
#         # Update UI for all views (e.g., icon and list)
#         for view in ['icon', 'list']:
#             row = current_data[f'{view}_row']           # The row widget
#             play_button = current_data[f'{view}_play_button']
#             options_button = current_data[f'{view}_options_button']

#             if current_data['is_selected']:
#                 # Row is selected: highlight it and show buttons
#                 row.add_css_class("blue")
#                 play_button.set_visible(True)
#                 options_button.set_visible(True)
#                 play_button.set_sensitive(True)  # Enable button
#                 options_button.set_sensitive(True)
#             else:
#                 # Row is unselected: remove highlight and hide buttons
#                 row.remove_css_class("blue")
#                 play_button.set_visible(False)
#                 options_button.set_visible(False)
#                 play_button.set_sensitive(False)  # Disable button
#                 options_button.set_sensitive(False)
#         # Toggle the selection state
#         #current_data['is_selected'] = not current_data.get('is_selected', False)
