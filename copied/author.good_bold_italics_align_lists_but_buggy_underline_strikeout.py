...
## /check format state of buttons and shortcuts
    def apply_persistent_formatting(self, format_type, enable):
        """Apply or remove formatting, with robust handling for underline and strikethrough."""
        script = f"""
            (function() {{
                let sel = window.getSelection();
                if (!sel.rangeCount) {{
                ...
                if (range.collapsed) {{
                    // Cursor only: prepare for next input
                    let span = document.createElement('span');
                    if ('{format_type}' === 'bold') {{
                        let currentState = document.queryCommandState('bold');
                        if (isEnabled !== currentState) {{
                            document.execCommand('bold', false, null);
                        }}
                        span.style.fontWeight = isEnabled ? 'bold' : 'normal';
                    }} else if ('{format_type}' === 'italic') {{
                        let currentState = document.queryCommandState('italic');
                        if (isEnabled !== currentState) {{
                            document.execCommand('italic', false, null);
                        }}
                        span.style.fontStyle = isEnabled ? 'italic' : 'normal';
                    }} else if ('{format_type}' === 'underline') {{
                        span.style.textDecoration = isEnabled ? 'underline' : 'none';
                    }} else if ('{format_type}' === 'strikethrough') {{
                        span.style.textDecoration = isEnabled ? 'line-through' : 'none';
                    }}
                    ...
                    // Selection exists: toggle formatting
                    if ('{format_type}' === 'bold' || '{format_type}' === 'italic') {{
                        let currentState = document.queryCommandState('{format_type}');
                        if (isEnabled !== currentState) {{
                            document.execCommand('{format_type}', false, null);
                        }}
                    }} else if ('{format_type}' === 'underline' || '{format_type}' === 'strikethrough') {{
                        let tag = '{format_type}' === 'underline' ? 'u' : 's';
                        let selectedNodes = [];
                        ...
                        if (isEnabled && selectedNodes.length === 0) {{
                            let wrapper = document.createElement(tag);
                            try {{
                                range.surroundContents(wrapper);
                            }} catch (e) {{
                                // Handle partial selection: extract and wrap
                                let contents = range.extractContents();
                                wrapper.appendChild(contents);
                                range.insertNode(wrapper);
                            }}
                        }} else if (!isEnabled && selectedNodes.length > 0) {{
                ...
                console.log('Applied {format_type}: ' + isEnabled);
            }})();
        """
        self.exec_js(script)
    ...
    def update_formatting_state(self):
        """Update toggle buttons based on current formatting state at cursor, respecting user intent."""
        script = """
            (function() {
                let sel = window.getSelection();
                ...
                let states = {
                    ...
                    underline: parent.closest('u') !== null || (parent.tagName === 'SPAN' && parent.style.textDecoration === 'underline'),
                    strikethrough: parent.closest('s') !== null || (parent.tagName === 'SPAN' && parent.style.textDecoration === 'line-through'),
