    ...
    def on_download_runner_response(self, dialog, response_id, combo_boxes, callback=None):
        ...
        if response_id == "download":
            ...
            if selected_runners:
                ...
                # Only show total_progress_bar if more than one runner is selected
    ...
    def download_runners_thread(self, selected_runners, progress_dialog, total_progress_bar,
        ...
        try:
            for idx, (label, runner) in enumerate(selected_runners.items()):
                ...
                GLib.idle_add(progress_label.set_text, f"Downloading {runner['name']} ({idx + 1}/{total_runners})...")
                GLib.idle_add(runner_progress_bar.set_fraction, 0.0)
                if total_runners > 1:
                    GLib.idle_add(total_progress_bar.set_fraction, idx / total_runners)
            ...
            if download_success and total_runners > 1:
                GLib.idle_add(total_progress_bar.set_fraction, 1.0)

        finally:
            if current_file and current_file.exists():
                current_file.unlink(missing_ok=True)

            def finalize_ui(message_title, message_body):
                # Wrap show_info_dialog in a lambda to ensure proper argument passing
                GLib.idle_add(
                    lambda: self.show_info_dialog(
                        message_title,
                        message_body,
                        callback  # Pass callback directly, handle it in show_info_dialog
                    )
                )
                # Delay closing progress_dialog until info dialog is shown
                GLib.timeout_add(500, lambda: progress_dialog.close() if not was_cancelled else None)
    ...
    def finalize_ui(message_title, message_body):
        GLib.idle_add(
            lambda: self.show_info_dialog(
                message_title,
                message_body,
                lambda: callback() if callback else None  # Callback executes after "OK"
            )
        )
