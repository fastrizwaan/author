            ...
            script = f"""
                (function() {{
                    ...
                    if (sel.rangeCount > 0) {{
                        ...
                        if (!range.collapsed) {{
                            document.execCommand('fontName', false, '{self.current_font}');
                        }} else {{
                            // Set up for typing
                            let span = document.createElement('span');
                            span.style.fontFamily = '{self.current_font}';
                            span.style.fontSize = '{self.current_font_size}pt';
        ...
        if item := dropdown.get_selected_item():
            ...
            script = f"""
                (function() {{
                    let sel = window.getSelection();
                    if (sel.rangeCount > 0) {{
                        let newSize = '{self.current_font_size}pt';
                        let ranges = [];
                        for (let i = 0; i < sel.rangeCount; i++) {{
                            ranges.push(sel.getRangeAt(i));
                        }}
                        // Process ranges in reverse to avoid DOM disruption
                        ...
                        // Restore selection across all modified spans
                        sel.removeAllRanges();
                        ranges.forEach((range, index) => {{
                            let newRange = document.createRange();
                            let span = range.startContainer.parentElement;
                            if (span.tagName === 'SPAN' && span.style.fontSize === newSize) {{
...
###### Fix for nested font size  text with something; something font changed, then select all, size changed; something size did not change. fix? removes nested sizes.
    def on_font_family_changed(self, dropdown, *args):
        if item := dropdown.get_selected_item():
            self.current_font = item.get_string()
            script = f"""
    ...
    def on_font_size_changed(self, dropdown, *args):
        if item := dropdown.get_selected_item():
            ...
            script = f"""
                (function() {{
                    ...
                    if (sel.rangeCount > 0 && !sel.isCollapsed) {{
                        ...
                        // Apply a temporary large size to mark the selection
                        document.execCommand('fontSize', false, '7');
                        // Find all elements with font-size keywords or font[size] from the selection
                        let root = document.body; // Adjust if your editable area is a specific element
                        let sizeElements = root.querySelectorAll('span[style*="font-size"], font[size]');
                        sizeElements.forEach(elem => {{
                            // Check if this element is within the original selection
                            let range = sel.getRangeAt(0);
            ...
            self.exec_js(script)
            self.update_formatting_ui()
###################### The above one fixed font selection, but only jumping cursor
    def on_font_size_changed(self, dropdown, *args):
        if item := dropdown.get_selected_item():
            self.current_font_size = item.get_string()
            script = f"""
                (function() {{
                    let sel = window.getSelection();
                    if (sel.rangeCount > 0 && !sel.isCollapsed) {{
                        // Store the selected text to find it later
                        ...
                        document.execCommand('styleWithCSS', false, true);
                        document.execCommand('fontSize', false, '7');
                        let root = document.body; // Adjust if your editable area is different
                        let sizeElements = root.querySelectorAll('span[style*="font-size"], font[size]');
                        sizeElements.forEach(elem => {{
                            if (range.intersectsNode(elem)) {{
                                let span = document.createElement('span');
                        ...
                        // Restore the selection by finding the modified text
                        ...
                        while (walker.nextNode()) {{
                            ...
                            if (node.nodeValue.includes(selectedText)) {{
                                let startIdx = node.nodeValue.indexOf(selectedText);
                                let endIdx = startIdx + selectedText.length;
                                newRange.setStart(node, startIdx);
                                newRange.setEnd(node, endIdx);
                                foundStart = true;
                                break;
                            }}
                        }}
    ...
    def on_font_family_changed(self, dropdown, *args):
        if item := dropdown.get_selected_item():
            ...
            self.exec_js(script)
            self.update_formatting_ui()
################################ for font size THREE, changing R's size messing up THR
    def on_font_size_changed(self, dropdown, *args):
        if item := dropdown.get_selected_item():
            self.current_font_size = item.get_string()
            script = f"""
                (function() {{
                    ...
                    if (sel.rangeCount > 0 && !sel.isCollapsed) {{
                        ...
                        // Apply the font size change to the exact selection
                        document.execCommand('styleWithCSS', false, true);
                        document.execCommand('fontSize', false, '7');

                        // Find and replace only the exact selected portion
                        let root = document.body;
                        let sizeElements = root.querySelectorAll('span[style*="font-size"], font[size]');
                        sizeElements.forEach(elem => {{
                            if (range.intersectsNode(elem)) {{
                                let elemText = elem.textContent;
                                let selectedPortion = selectedText;
                                let beforeText = elemText.substring(0, startOffset);
                                let afterText = elemText.substring(endOffset);

                                let newSpan = document.createElement('span');
                                newSpan.style.fontSize = '{self.current_font_size}pt';
                                newSpan.textContent = selectedPortion;

                                // Reconstruct the element with before, selected, and after parts
                                elem.innerHTML = '';
                                if (beforeText) elem.appendChild(document.createTextNode(beforeText));
                                elem.appendChild(newSpan);
                                if (afterText) elem.appendChild(document.createTextNode(afterText));
                            }}
                        }});
        ...
        if item := dropdown.get_selected_item():
            ...
            script = f"""
                (function() {{
                    let sel = window.getSelection();
                    if (sel.rangeCount > 0 && !sel.isCollapsed) {{
                        // More precise selection handling
                        let range = sel.getRangeAt(0);
                        
                        // First, save the exact range details
                        let startContainer = range.startContainer;
                        let startOffset = range.startOffset;
                        let endContainer = range.endContainer;
                        let endOffset = range.endOffset;
                        
                        // Use surroundContents for precise wrapping
                        document.execCommand('styleWithCSS', false, true);
                        
                        // If we're dealing with a complex selection, we need to create a new span
            ...
            self.exec_js(script)
            self.update_formatting_ui()
##############################/CL below fixed, but loses selection THREE R then THREE fixed
    def on_font_size_changed(self, dropdown, *args):
        if item := dropdown.get_selected_item():
            self.current_font_size = item.get_string()
            script = f"""
                (function() {{
                    let sel = window.getSelection();
                    if (sel.rangeCount > 0 && !sel.isCollapsed) {{
                        // Save the exact range
                        let originalRange = sel.getRangeAt(0).cloneRange();
                        let originalContent = originalRange.cloneContents();
                        
                        // Execute the font size command
                        document.execCommand('styleWithCSS', false, true);
                        document.execCommand('fontSize', false, '7');
                        
                        // Find only the elements that were just created or modified
                        // specifically with fontSize=7 (WebKit size) and are within our selection
                        let affectedElements = [];
                        let findNewlyCreatedFontElements = (node) => {{
                            if (node.nodeType === 1) {{ // Element node
                                // Check if it's a font element with size=7 or span with font-size
                                if ((node.nodeName === 'FONT' && node.getAttribute('size') === '7') ||
                                    (node.nodeName === 'SPAN' && node.style.fontSize && 
                                     (node.style.fontSize === '7' || node.style.fontSize === 'xxx-large'))) {{
                                    affectedElements.push(node);
                                }} else {{
                                    // Check children
                                    for (let i = 0; i < node.childNodes.length; i++) {{
                                        findNewlyCreatedFontElements(node.childNodes[i]);
                                    }}
                                }}
                            }}
                        }};
                        ...
                        let commonAncestor = originalRange.commonAncestorContainer;
                        if (commonAncestor.nodeType === 3) {{ // If it's a text node, get its parent
                            commonAncestor = commonAncestor.parentNode;
                        }}
                        
                        // Find all newly created font elements
                        findNewlyCreatedFontElements(commonAncestor);
                        
                        // Convert each found element to our precise pt size
                        affectedElements.forEach(elem => {{
                            ...
                            if (elem.style) {{
                                if (elem.style.fontFamily) newSpan.style.fontFamily = elem.style.fontFamily;
                                if (elem.style.fontWeight) newSpan.style.fontWeight = elem.style.fontWeight;
                                if (elem.style.fontStyle) newSpan.style.fontStyle = elem.style.fontStyle;
                            }}
                            
                            // Copy any other attributes
                            for (let i = 0; i < elem.attributes.length; i++) {{
                                let attr = elem.attributes[i];
                                if (attr.name !== 'style' && attr.name !== 'size') {{
                            ...
                            while (elem.firstChild) {{
                                newSpan.appendChild(elem.firstChild);
                            }}
                            
                            // Replace the element
                            elem.parentNode.replaceChild(newSpan, elem);
                        }});
                        ...
                        try {{
                            ...
                            if (affectedElements.length > 0) {{
                                newRange.selectNode(affectedElements[0].parentNode);
                                sel.addRange(newRange);
                            }} else {{
        ...
        if item := dropdown.get_selected_item():
            ...
            script = f"""
                (function() {{
                    let sel = window.getSelection();
                    if (sel.rangeCount > 0 && !sel.isCollapsed) {{
                        // Save the exact range and selected text
                        let originalRange = sel.getRangeAt(0).cloneRange();
                        let selectedText = sel.toString();
                        ...
                        // Find newly created/modified elements within the selection
                        let affectedElements = [];
                        let findNewlyCreatedFontElements = (node) => {{
                            if (node.nodeType === 1) {{ // Element node
                                if ((node.nodeName === 'FONT' && node.getAttribute('size') === '7') ||
                        ...
                        // Convert affected elements to precise pt size and track new spans
                        let newSpans = [];
                        affectedElements.forEach(elem => {{
                            let newSpan = document.createElement('span');
                            newSpan.style.fontSize = '{self.current_font_size}pt';
                        ...
                        // Restore selection to the new span containing the original text
                        if (newSpans.length > 0) {{
                            sel.removeAllRanges();
                            let newRange = document.createRange();
                            let targetSpan = newSpans.find(span => span.textContent === selectedText);
                            if (targetSpan) {{
                                newRange.selectNodeContents(targetSpan);
                            }} else {{
                                // Fallback: Select the first new span if exact match fails
