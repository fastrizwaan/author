...
<body><p>&#8203;</p></body>
    ...
    def on_webview_clicked(self, gesture, n_press, x, y):
        """Handle click events in the WebView, update button states based on clicked text formatting."""
        if n_press == 1:
            print(f"Single click at x={x}, y={y}")
        elif n_press == 2:
            print(f"Double click at x={x}, y={y}")
        ...
        script = f"""
            (function() {{
                ...
                if (isText) {{
                    ...
                    if ({n_press} === 2 && sel.toString().length === 0) {{
                        // Double-click: Let WebView handle word selection, no custom range needed
                        return true;
                    }} else if ({n_press} === 1) {{
                        let range = document.createRange();
                        if (element.nodeType === 3) {{
                            let offset = Math.min(Math.round({x} / 10), element.length);
    ...
    def on_click_check_result(self, webview, result, user_data):
        """Handle result of click check and update button states if text was clicked."""
        try:
            js_value = webview.evaluate_javascript_finish(result)
            if js_value and js_value.is_boolean() and js_value.to_boolean():
                print("Text clicked, updating button states")
                self.update_formatting_state()  # Update buttons only
            else:
        ...
        try:
            ...
            if js_value:
                raw_html = js_value.to_string()
                    # GLib.Bytes.new(raw_html.encode()),
...
#########################
    def apply_persistent_formatting(self, format_type, enable):
        """Apply or remove formatting for new text or selected text with proper toggling."""
        # Update state
        if format_type == 'bold':
            self.is_bold = enable
        elif format_type == 'italic':
        ...
        script = f"""
            (function() {{
                ...
                // First, clean up any existing format markers at the current position
                function cleanFormatMarkers() {{
                    // If cursor is inside a format marker, we need to clean up
                    let node = range.startContainer;
                    let parent = node.parentNode;
                    
                    // Check if we're in a text node inside a format marker
                    if (node.nodeType === 3 && parent && parent.className === 'format-marker') {{
                        // We're inside a format marker, check if we need to unwrap
                        if (range.startOffset === node.length) {{
                            // If at the end of the marker, fix nested markers
                            let markers = [];
                            let current = parent;
                            
                            // Collect all nested format markers
                            while (current && current.className === 'format-marker') {{
                                markers.push(current);
                                current = current.parentNode;
                            }}
                            
                            if (markers.length > 1) {{
                                // We have nested markers, consolidate them
                                let topMarker = markers[markers.length - 1];
                                let textNode = document.createTextNode('​'); // Zero-width space
                                
                                // Create a single new marker with current format state
                                let newMarker = document.createElement('span');
                                newMarker.className = 'format-marker';
                                newMarker.style.fontWeight = {("'bold'" if self.is_bold else "'normal'")};
                                newMarker.style.fontStyle = {("'italic'" if self.is_italic else "'normal'")};
                                newMarker.style.textDecoration = '{decoration_style}';
                                newMarker.appendChild(textNode);
                                
                                // Insert the new marker after the top marker
                                if (topMarker.nextSibling) {{
                                    topMarker.parentNode.insertBefore(newMarker, topMarker.nextSibling);
                                }} else {{
                                    topMarker.parentNode.appendChild(newMarker);
                                }}
                                
                                // Move the cursor to the new marker
                                range = document.createRange();
                ...
                // Try to clean format markers
                let cleaned = cleanFormatMarkers();
                
                // Apply formatting commands based on type
                if (!cleaned) {{
                    if ('{format_type}' === 'bold') {{
                        if ({str(enable).lower()} !== document.queryCommandState('bold')) {{
                            document.execCommand('bold', false, null);
                    ...
                    if (range.collapsed) {{
                        // Create a single span with current formatting state for future typing
                        let formatSpan = document.createElement('span');
                        formatSpan.className = 'format-marker';
                        formatSpan.style.fontWeight = {("'bold'" if self.is_bold else "'normal'")};
                        formatSpan.style.fontStyle = {("'italic'" if self.is_italic else "'normal'")};
                        formatSpan.style.textDecoration = '{decoration_style}';
                        
                        // Insert zero-width space to hold the cursor
                        formatSpan.innerHTML = '​'; // Zero-width space
                        range.insertNode(formatSpan);
                        
                        // Set cursor position inside the formatting span
                        range = document.createRange();
                        range.setStart(formatSpan.firstChild, 1);
                        range.setEnd(formatSpan.firstChild, 1);
                        sel.removeAllRanges();
                        sel.addRange(range);
                    }} else {{
                        // Selection exists: consolidate styles into a single span
                        let contents = range.extractContents();
                        if (contents.textContent.length > 0) {{
                            // Find and remove any nested format markers in the extracted content
                            let formatMarkers = contents.querySelectorAll('.format-marker');
                            for (let marker of formatMarkers) {{
                                // Replace marker with its text content
                                let parent = marker.parentNode;
                                let textContent = marker.textContent;
                                let textNode = document.createTextNode(textContent);
                                parent.replaceChild(textNode, marker);
                            }}
                            
                            // Create a single span with current state
                            let span = document.createElement('span');
                            span.style.fontWeight = {("'bold'" if self.is_bold else "'normal'")};
                            span.style.fontStyle = {("'italic'" if self.is_italic else "'normal'")};
                            span.style.textDecoration = '{decoration_style}';
                ...
                console.log('Applied formatting - bold: {self.is_bold}, italic: {self.is_italic}, underline: {self.is_underline}, strikethrough: {self.is_strikethrough}, decoration: {decoration_style}');
            }})();
        """
        self.exec_js(script)
############### cl3
    def apply_persistent_formatting(self, format_type, enable):
        """Apply or remove formatting with cleaner HTML structure."""
        # Update state
        if format_type == 'bold':
            self.is_bold = enable
        elif format_type == 'italic':
        ...
        script = f"""
            (function() {{
                ...
                // Function to clean up format markers
                function normalizeFormatting(node) {{
                    if (!node) return;
                    
                    // Find all format markers within this node
                    let markers = node.querySelectorAll('.format-marker');
                    for (let marker of markers) {{
                        // If it contains another format marker, flatten the hierarchy
                        let nestedMarkers = marker.querySelectorAll('.format-marker');
                        if (nestedMarkers.length > 0) {{
                            // Move all text out of nested markers
                            let textContent = marker.textContent;
                            // Remove all children
                            while (marker.firstChild) {{
                                marker.removeChild(marker.firstChild);
                            }}
                            // Add back just the text
                            marker.textContent = textContent;
                        }}
                    }}
                    
                    // Combine adjacent format markers with identical formatting
                    let current = node.firstChild;
                    while (current && current.nextSibling) {{
                        let next = current.nextSibling;
                        
                        if (current.className === 'format-marker' && 
                            next.className === 'format-marker' &&
                            current.style.fontWeight === next.style.fontWeight &&
                            current.style.fontStyle === next.style.fontStyle &&
                            current.style.textDecoration === next.style.textDecoration) {{
                ...
                if (range.collapsed) {{
                    // First check if we're inside an existing format marker
                    let node = range.startContainer;
                    let parent = node.parentNode;
                    
                    if (node.nodeType === 3 && parent && parent.className === 'format-marker') {{
                        // Update the existing marker instead of creating a new one
                        parent.style.fontWeight = {("'bold'" if self.is_bold else "'normal'")};
                        parent.style.fontStyle = {("'italic'" if self.is_italic else "'normal'")};
                        parent.style.textDecoration = '{decoration_style}';
                    }} else {{
                        // Create a new format marker
                        let formatSpan = document.createElement('span');
                        formatSpan.className = 'format-marker';
                        formatSpan.style.fontWeight = {("'bold'" if self.is_bold else "'normal'")};
                        formatSpan.style.fontStyle = {("'italic'" if self.is_italic else "'normal'")};
                        ...
                        // Insert zero-width space to hold the cursor
                        formatSpan.textContent = '​'; // Zero-width space
                        range.insertNode(formatSpan);
                    ...
                    // Selection exists: replace with a single clean span
                    let contents = range.extractContents();
                    
                    // Create a temporary div to hold and clean the contents
                    let tempDiv = document.createElement('div');
                    tempDiv.appendChild(contents);
                    
                    // Clean up any nested format markers
                    normalizeFormatting(tempDiv);
                    
                    // Create a new span with the current formatting
                    let span = document.createElement('span');
                    span.className = 'format-marker';
                    span.style.fontWeight = {("'bold'" if self.is_bold else "'normal'")};
                    span.style.fontStyle = {("'italic'" if self.is_italic else "'normal'")};
                    span.style.textDecoration = '{decoration_style}';
                    
                    // Move the cleaned content to the new span
                    span.textContent = tempDiv.textContent;
                    
                    // Insert the clean span
                    range.insertNode(span);
                ...
                // Clean up the entire paragraph to reduce nesting
                let paragraph = range.startContainer;
                while (paragraph && paragraph.nodeName !== 'P') {{
                    paragraph = paragraph.parentNode;
                }}
                if (paragraph) {{
                    normalizeFormatting(paragraph);
                }}

                console.log('Applied formatting with clean structure - bold: {self.is_bold}, italic: {self.is_italic}, underline: {self.is_underline}, strikethrough: {self.is_strikethrough}');
            }})();
        """
        self.exec_js(script)
    ...
    def update_formatting_state(self):
        """Update button states based on current cursor position's formatting."""
        script = """
            (function() {
                let sel = window.getSelection();
    ...
    def on_formatting_state_received(self, webview, result, user_data):
        """Update button states based on received formatting states."""
        try:
            js_value = webview.evaluate_javascript_finish(result)
            if js_value and js_value.is_string():
                states = json.loads(js_value.to_string())
                # Update button states only, without modifying document
                self.bold_btn.set_active(states.get('bold', False))
                self.italic_btn.set_active(states.get('italic', False))
                self.underline_btn.set_active(states.get('underline', False))
                self.strikethrough_btn.set_active(states.get('strikethrough', False))
                self.bullet_btn.set_active(states.get('ul', False))
                self.number_btn.set_active(states.get('ol', False))
                
                # Update alignment buttons
                self.align_left_btn.set_active(states.get('justifyLeft', False))
                self.align_center_btn.set_active(states.get('justifyCenter', False))
                self.align_right_btn.set_active(states.get('justifyRight', False))
                self.align_justify_btn.set_active(states.get('justifyFull', False))
                
                # Update internal state variables to match button states
                self.is_bold = states.get('bold', False)
                self.is_italic = states.get('italic', False)
                self.is_underline = states.get('underline', False)
                self.is_strikethrough = states.get('strikethrough', False)
                self.is_bullet_list = states.get('ul', False)
                self.is_number_list = states.get('ol', False)
                self.is_align_left = states.get('justifyLeft', False)
                self.is_align_center = states.get('justifyCenter', False)
                self.is_align_right = states.get('justifyRight', False)
                self.is_align_justify = states.get('justifyFull', False)
                
                print(f"Button states updated: bold={self.is_bold}, italic={self.is_italic}, "
                    f"underline={self.is_underline}, strikethrough={self.is_strikethrough}, "
                    f"bullet={self.is_bullet_list}, number={self.is_number_list}, "
                    f"align_left={self.is_align_left}, align_center={self.is_align_center}, "
                    f"align_right={self.is_align_right}, align_justify={self.is_align_justify}")
        except Exception as e:
...
## /check format state of buttons and shortcuts
    def clean_html(self, html, filename=None):
        ...
        # Replace \xa0 and \u200B with regular spaces
        ...
        # Define style comparison function
        ...
        # Flatten only truly redundant nested spans (same style, no additional children)
        for span in soup.find_all('span', recursive=True):
            parent = span.parent
            if (parent.name == 'span' and 
                spans_are_equivalent(span, parent) and 
                not span.find_all(recursive=False)):  # No child elements
                parent.string = span.get_text().strip()
                span.decompose()

        # Minimize over-consolidation by preserving original span boundaries
        for block in soup.find_all(['p', 'div', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'li']):
            ...
            while i < len(spans) - 1:
                ...
                # Check if next_sibling exists and is either a span or empty text
                can_merge = (spans_are_equivalent(current_span, next_span) and
                            (next_sibling is None or 
                            (isinstance(next_sibling, str) and not next_sibling.strip()) or 
                            next_sibling == next_span))
                if can_merge:
                    current_span.string = (current_span.get_text() + next_span.get_text()).strip()
                    next_span.decompose()
                    spans.pop(i + 1)
                else:
                    i += 1

        # Only unwrap spans with no meaningful style AND no text
        for span in soup.find_all('span'):
        ...
        html_output = (
            ...
            '</style>\n'
            '</head>\n'
            f'<body>\n{str(soup.body)[6:-7] if soup.body else ""}\n</body>\n'
            '</html>'
        )
...
################### ch
    ...
    def clean_html(self, html, filename=None):
        """Clean up HTML for HTML5 compliance while preserving essential formatting."""
        soup = BeautifulSoup(html, 'html.parser')

        # Replace zero-width spaces (\u200B) with nothing, preserve non-breaking spaces (\xa0)
        for text in soup.find_all(string=True):
            if text.parent.name not in ['style', 'script']:
        ...
        # Remove empty spans or those containing only <br>
        for span in soup.find_all('span'):
            contents = [child for child in span.contents if not (isinstance(child, str) and child.strip() == '')]
            if not contents or all(tag.name == 'br' for tag in contents):
                span.decompose()
        ...
        def get_style_dict(tag):
            style = tag.get('style', '')
            return dict(item.strip().split(': ') for item in style.split(';') if ':' in item)

        def spans_are_equivalent(span1, span2):
            style1 = get_style_dict(span1)
            style2 = get_style_dict(span2)
            return style1 == style2  # Ensure exact style match before merging

        def is_default_style(style_dict):
            return (style_dict.get('font-weight') == 'normal' and
                    style_dict.get('font-style') == 'normal' and
                    style_dict.get('text-decoration') == 'none')

        def merge_nested_spans(tag):
            """Recursively merge nested spans with identical styles."""
            ...
            while i < len(spans):
                current_span = spans[i]
                merge_nested_spans(current_span)  # Recursively clean nested spans
                next_sibling = current_span.next_sibling
                if isinstance(next_sibling, Tag) and next_sibling.name == 'span':
                    next_span = next_sibling
                    if spans_are_equivalent(current_span, next_span):
                        current_span.append(next_span.get_text())
                        next_span.decompose()
                        spans.pop(i + 1)
                        continue
                # Unwrap nested spans with identical styles
                nested_spans = current_span.find_all('span', recursive=False)
                for nested in nested_spans:
                    if spans_are_equivalent(current_span, nested):
                        current_span.append(nested.get_text())
                        nested.decompose()
                i += 1

        # Merge spans and simplify redundant ones
        for block in soup.find_all(['p', 'div', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'li']):
            merge_nested_spans(block)

        # # Remove spans with default styling unless they have a class
        # for span in soup.find_all('span'):
        #     style_dict = get_style_dict(span)
        #     if is_default_style(style_dict) and not span.attrs.get('class'):
        #         span.unwrap()

        # Remove empty block elements
        for block in soup.find_all(['p', 'div', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'li']):
            if not block.get_text(strip=True) and not block.find_all(recursive=False):
                block.decompose()
        ...
        # Construct HTML5 output with preserved formatting
        html_output = (
            '<!DOCTYPE html>\n'
            '<html>\n'
            '<head>\n'
    ...
    def save_html_callback(self, webview, result, file):
        ...
        try:
            ...
            if js_value:
                raw_html = js_value.to_string()
                filename = file.get_path() if hasattr(file, 'get_path') else str(file)
                cleaned_html = self.clean_html(raw_html, filename)
                file.replace_contents_bytes_async(
                    GLib.Bytes.new(cleaned_html.encode()),
                    None,
                    False,
