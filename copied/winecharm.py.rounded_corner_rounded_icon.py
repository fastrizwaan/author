...
###################### 0.95
    ...
    def create_script_row(self, script_key, script_data):
        ...
        if self.icon_view:
            ...
            # Label area with text wrapping
            label_text = str(script_data.get('progname', script.stem)).replace('_', ' ')
            label_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
            #label_box.set_margin_start(4)
            #label_box.set_margin_end(4)
            label_box.set_halign(Gtk.Align.CENTER)
            label_box.set_valign(Gtk.Align.CENTER)

            main_label = Gtk.Label()
            main_label.set_wrap(True)
            main_label.set_max_width_chars(20)  # Wrap after ~10 characters
            main_label.set_lines(2)  # Max 2 lines
            main_label.set_ellipsize(Pango.EllipsizeMode.END)  # Add ... if too long
            main_label.set_halign(Gtk.Align.CENTER)
            ...
            container.set_size_request(40, 40)  # Width x Height in pixels
            container.append(label_box)
    ...
    def load_icon(self, script, x, y):
        ...
        if pixbuf:
            ...
            # Convert to texture
            texture = Gdk.Texture.new_for_surface(surface)
        else:
            texture = None
