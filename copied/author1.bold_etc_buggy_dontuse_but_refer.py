    ...
    def apply_persistent_formatting(self, format_type, enable):
        ...
        script = f"""
            (function() {{
                ...
                // If disabling bold, ensure new content isn't bold
                if (cmd === 'bold' && !{desired}) {{
                    // Insert a span with default styling at cursor
                    let span = document.createElement('span');
                    ...
                    sel.removeAllRanges();
                    sel.addRange(range);
                    // Clean up empty spans later
                    setTimeout(() => {{
                        if (span.innerHTML === '&#8203;' && !span.nextSibling) {{
                            span.remove();
                        }}
                    }}, 0);
                }}
    ...
    def apply_persistent_formatting(self, format_type, enable):
        ...
        script = f"""
            (function() {{
                ...
                // If disabling formatting, ensure new content isn't formatted
                if (!{desired}) {{
                    // Insert a span with default styling at cursor
                    let span = document.createElement('span');
                    
                    // Apply appropriate style reset based on format type
                    if (cmd === 'bold') {{
                        span.style.fontWeight = 'normal';
                    }} else if (cmd === 'italic') {{
        ...
        """
        self.exec_js(script)
# above works with bold and italics
    def apply_persistent_formatting(self, format_type, enable):
        """Apply or remove formatting so that subsequently typed text adopts (or loses) the style."""
        desired = str(enable).lower()  # yields "true" or "false"
        script = f"""
            (function() {{
                ...
                // If we're disabling and it's currently on, we need special handling
                if (currentState && !{desired}) {{
                    // Insert an empty character with all formatting reset
                    let span = document.createElement('span');
                    span.style.all = 'unset'; // Reset all formatting
                    
                    // Specific resets for each format type
                    span.style.fontWeight = 'normal';
                    span.style.fontStyle = 'normal';
                    span.style.textDecoration = 'none';
                    ...
                    // Turn off the command explicitly
                    document.execCommand(cmd, false, null);
                    
                    // Listen for the next character input to remove empty span
                    document.addEventListener('input', function removeEmptySpan() {{
                        // Get spans with zero-width space
                        const spans = document.querySelectorAll('span:empty, span:contains("​")');
                        spans.forEach(s => {{
                            if (!s.textContent || s.textContent === '​') {{
                                s.remove();
                            }}
                        }});
                        document.removeEventListener('input', removeEmptySpan);
                    }}, {{ once: true }});
                }} else {{
                    // Simple toggle when enabling or already disabled
                    if (currentState !== {desired}) {{
                        document.execCommand(cmd, false, null);
                    }}
                }}
        ...
        """
        self.exec_js(script)
# The above resets all when enter is typed
    # def apply_persistent_formatting(self, format_type, enable):
    #     """Apply or remove formatting so that subsequently typed text adopts (or loses) the style."""
    #     desired = str(enable).lower()  # yields "true" or "false"
    #     script = f"""
    ...
    #                     span.style.fontStyle = 'normal';
    #                 }} else if (cmd === 'underline') {{
    #                     // Specific handler for underline
    #                     span.style.textDecorationLine = 'none';
    #                     span.style.textDecoration = 'none';
    #                 }} else if (cmd === 'strikeThrough') {{
    #                     // Specific handler for strikethrough
    #                     span.style.textDecorationLine = 'none';
    #                     span.style.textDecoration = 'none';
    #                 }}
    ...
    #     """
    #     self.exec_js(script)        
############# Q bold, italics + textdecorator works, but underline+strikeout fails
    def apply_persistent_formatting(self, format_type, enable):
        """Apply or remove formatting so that subsequently typed text adopts (or loses) the style."""
        desired = str(enable).lower()  # yields "true" or "false"
        script = f"""
            (function() {{
                ...
                if (currentState && !{desired}) {{
                    ...
                    document.addEventListener('input', function removeEmptySpan() {{
                        // Get spans with zero-width space
                        const spans = document.querySelectorAll('span:empty, span');
                        spans.forEach(s => {{
                            if (!s.textContent || s.textContent === '​') {{ // Zero-width space
                                s.remove();
                            }}
                        }});
                        document.removeEventListener('input', removeEmptySpan);
                ...
                // Add special handling for Enter key to preserve formatting
                if ({desired}) {{
                    // Store the current formatting state to be applied after Enter
                    document.addEventListener('keydown', function handleEnterKey(e) {{
                        if (e.key === 'Enter') {{
                            // Remember current formatting state
                            let wasFormatActive = document.queryCommandState(cmd);
                            
                            if (wasFormatActive) {{
                                // Set a timeout to reapply after the new paragraph is created
                                setTimeout(() => {{
                                    // Only reapply if it was active and is now inactive
                                    if (wasFormatActive && !document.queryCommandState(cmd)) {{
                                        document.execCommand(cmd, false, null);
                                    }}
                                }}, 0);
                            }}
                        }}
                    }}, {{ once: true }}); // Only do this once for the next Enter
                }}
    ...
    def apply_persistent_formatting(self, format_type, enable):
        ...
        script = f"""
            (function() {{
                ...
                // If we're disabling and it's currently on, we need special handling
                if (currentState && !{desired}) {{
                    // Insert an empty character with specific formatting reset
                    let span = document.createElement('span');
