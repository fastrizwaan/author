...
<head>
    <style>
        ...
        @media (prefers-color-scheme: light) { 
            body { background-color: #f6f5f4; color: #000000; } 
        }
        ...
        .page {
            ...
            padding: 96px;   /* 1 inch margin on all sides */
            position: relative;
            box-sizing: border-box;
        }
        .content {
            height: 864px;   /* 1056px - 96px top - 96px bottom */
            outline: none;
            /* Remove overflow: hidden to allow height calculation */
        }
        @media (prefers-color-scheme: dark) {
        ...
        if load_event == WebKit.LoadEvent.FINISHED:
            script = """
                ...
                function createNewPage() {
                    ...
                    newContent.addEventListener('input', handleContentChange);
                    return newContent;
                }

                function handleContentChange() {
                    // Use requestAnimationFrame to wait for DOM to update
                    window.cancelAnimationFrame(window.pageBreakFrameRequest);
                    window.pageBreakFrameRequest = window.requestAnimationFrame(() => {
                        reorganizeContent();
                    });
                }
                ...
                function reorganizeContent() {
                    ...
                    document.querySelectorAll('.content').forEach(content => {
                        while (content.firstChild) {
                            allContent.appendChild(content.firstChild);
                        }
                    });
                    
                    // Keep only the first page, remove others
                    const pages = document.querySelectorAll('.page');
                    for (let i = pages.length - 1; i > 0; i--) {
                        pages[i].remove();
                    }
                    
                    // Reset the first page's content
                    const firstPageContent = pages[0].querySelector('.content');
                    firstPageContent.innerHTML = '';
                    
                    // Redistribute content
                    distributeContent(allContent, firstPageContent);
                }

                function distributeContent(sourceContent, targetContent) {
                    const contentHeightLimit = 864; // Maximum content height per page
                    ...
                    while (sourceContent.firstChild) {
                        ...
                        if (targetContent.scrollHeight > contentHeightLimit) {
                            // Remove the last added node
                            const overflowNode = targetContent.lastChild;
                            targetContent.removeChild(overflowNode);
                            
                            // Create a new page for overflow
                            const newContent = createNewPage();
                            
                            // Continue distribution with remaining content
                            const remainingContent = document.createElement('div');
                            remainingContent.appendChild(overflowNode);
                            while (sourceContent.firstChild) {
                                remainingContent.appendChild(sourceContent.firstChild);
                            }
                            
                            distributeContent(remainingContent, newContent);
                            break;
                        }
                    }
                }
                ...
                function initializeEditor() {
                    // Attach event listeners to the initial content div
                    const initialContent = document.querySelector('.content');
                    if (initialContent) {
                        initialContent.addEventListener('input', handleContentChange);
                    }
                    
                    // Make functions globally available
                    window.createNewPage = createNewPage;
                    window.reorganizeContent = reorganizeContent;
                    window.handleContentChange = handleContentChange;
                    
                    // Set up mutation observer for changes that might not trigger input events
                ...
                initializeEditor(); // Also run immediately in case page is already loaded
            """
        ...
        try:
            ...
            if text:
                ...
                script = f"""
                    ...
                    if (typeof window.handleContentChange === 'function') {{
