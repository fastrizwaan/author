            ...
            enhanced_script = """
                (function() {
                    ...
                    style.textContent = `
                        ...
                        .dragging {
                            opacity: 0; /* Fully hide during drag */
                        }
                        .context-menu {
                            position: absolute;
                            background: white;
                    ...
                    // Make all resizable images and textboxes draggable
                    document.querySelectorAll('.resizable-image, .textbox').forEach(el => {
                    ...
                    document.body.addEventListener('dragstart', function(e) {
                        let target = e.target;
                        if (target.classList.contains('textbox') || target.classList.contains('resizable-image')) {
                            e.dataTransfer.setData('text/plain', target.outerHTML);
                            target.classList.add('dragging');
                            // Calculate offset from where the drag starts
                            let rect = target.getBoundingClientRect();
                            let offsetX = e.clientX - rect.left;
                            let offsetY = e.clientY - rect.top;
                            target.dataset.dragX = offsetX;
                            target.dataset.dragY = offsetY;
                            // Set the drag image to the target element
                    ...
                    document.body.addEventListener('dragover', function(e) {
                        ...
                        e.dataTransfer.dropEffect = 'move'; // Indicate move operation
                    });

                    document.body.addEventListener('drop', function(e) {
                        e.preventDefault();
                        let target = document.querySelector('.dragging');
                        if (target && (target.classList.contains('textbox') || target.classList.contains('resizable-image'))) {
                            target.classList.remove('dragging');
                            target.style.visibility = 'visible'; // Restore visibility
                            // Update position based on drop location
                            let newX = e.clientX - parseFloat(target.dataset.dragX);
                            let newY = e.clientY - parseFloat(target.dataset.dragY);
                            // Ensure the element stays within bounds
                            newX = Math.max(0, Math.min(newX, document.body.offsetWidth - target.offsetWidth));
                            newY = Math.max(0, Math.min(newY, document.body.offsetHeight - target.offsetHeight));
                    ...
                    document.body.addEventListener('dragend', function(e) {
                        let target = e.target;
                        if (target.classList.contains('textbox') || target.classList.contains('resizable-image')) {
                            // Ensure visibility is restored if drop doesn't fire
                    ...
                    document.body.addEventListener('contextmenu', function(e) {
                        ...
                        if (target.classList.contains('textbox') || target.classList.contains('resizable-image')) {
                            ...
                            menu.innerHTML = `
                                <div onclick="window.adjustSize('${target.className}', 'resize')">Resize</div>
                                <div onclick="window.adjustSize('${target.className}', 'move')">Move</div>
                                <div onclick="window.alignElement('${target.className}', 'left')">Align Left</div>
                                <div onclick="window.alignElement('${target.className}', 'center')">Align Center</div>
                                <div onclick="window.alignElement('${target.className}', 'right')">Align Right</div>
                                <div onclick="window.toggleTextFlow('${target.className}')">Toggle Text Flow</div>
                            `;
                    ...
                    window.adjustSize = function(className, action) {
                        let element = activeElement;
                        if (action === 'resize') {
                            element.focus(); // Let user resize manually
                        } else if (action === 'move') {
                            element.style.cursor = 'move';
                        }
                    };

                    window.alignElement = function(className, align) {
                        let element = activeElement;
                        if (align === 'left') {
                    ...
                    window.toggleTextFlow = function(className) {
                        let element = activeElement;
