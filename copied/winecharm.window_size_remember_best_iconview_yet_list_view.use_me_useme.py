...
class WineCharmApp(Gtk.Application):
    ...
    def save_settings(self):
        ...
        # Get window size correctly for GTK4
        window_width = 880  # Default
        window_height = 810  # Default
        
        if self.window:
            # Get the actual current size of the window
            current_width = self.window.get_width()
            current_height = self.window.get_height()
            
            # Only use current dimensions if they're valid (greater than 0)
            if current_width > 0:
    ...
    def load_settings(self):
        ...
        if self.settings_file.exists():
            ...
            self.window_width = settings.get('window_width', 880)  # Default to 880 if not set
            self.window_height = settings.get('window_height', 810)  # Default to 810 if not set
            return settings
    ...
    def create_main_window(self):
        ...
        width = settings.get('window_width', 880)
        height = settings.get('window_height', 810)
...
###################### 0.95
    def create_script_row(self, script_key, script_data):
        ...
        if self.icon_view:
            ...
            # Options button (wide, consistent, initially hidden)
            options_button = Gtk.Button(icon_name="emblem-system-symbolic", tooltip_text="Options")
            options_button.add_css_class("flat")
            options_button.set_size_request(60, 34)  # Explicitly wide
            options_button.set_hexpand(False)  # Prevent over-expansion, rely on size request
            options_button.set_opacity(0)  # Hidden by default
            options_button.set_sensitive(False)
            # Ensure button content doesn't shrink it
            options_button.set_margin_start(10)
            options_button.set_margin_end(10)
            top_box.append(options_button)

            # Icon (centered)
            icon = self.load_icon(script, 96, 96, 10)
            icon_container = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=0)
            icon_container.set_hexpand(True)  # Icon takes remaining space
            ...
            # Play button (wide, consistent, initially hidden)
            play_button = Gtk.Button(icon_name="media-playback-start-symbolic", tooltip_text="Play")
            play_button.add_css_class("flat")
            play_button.set_size_request(60, 34)  # Explicitly wide, matching options_button
            play_button.set_hexpand(False)  # Prevent over-expansion, rely on size request
...
####################################
    def create_script_row(self, script_key, script_data):
        ...
        if self.icon_view:
            ...
            options_button = Gtk.Button(icon_name="emblem-system-symbolic", tooltip_text="Options")
            options_button.add_css_class("flat")
            options_button.set_size_request(50, -1)  # Consistent width with play button
            options_button.set_hexpand(False)
            ...
            icon = self.load_icon(script, 96, 96, 10)
            icon_container = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=0)
            icon_container.set_hexpand(False)  # Allow icon to expand in the center
            icon_container.set_halign(Gtk.Align.CENTER)
            #icon_container.set_valign(Gtk.Align.CENTER)
            if icon:
                icon_container.add_css_class("rounded-icon")
            ...
            play_button = Gtk.Button(icon_name="media-playback-start-symbolic", tooltip_text="Play")
            play_button.add_css_class("flat")
            play_button.set_size_request(50, -1)  # Consistent width with options button
            play_button.set_opacity(0)  # Hidden by default
            play_button.set_sensitive(False)
