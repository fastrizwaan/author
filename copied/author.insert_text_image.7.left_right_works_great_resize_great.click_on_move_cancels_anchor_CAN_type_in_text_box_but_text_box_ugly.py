...
<body><p>&nbsp;</p></body>
    ...
    def on_webview_load(self, webview, load_event):
        if load_event == WebKit.LoadEvent.FINISHED:
            ...
            # Enhanced script with free dragging, resizing, and proper textbox editing/select-all
            enhanced_script = """
                (function() {
                    // Debounce and notifyChange
                    ...
                    ['input','keyup','paste','cut'].forEach(evt => {
                        document.addEventListener(evt, notifyChange);
                    });
                    new MutationObserver(notifyChange).observe(document.body, {childList: true, subtree: true, characterData: true, attributes: true});
                    ...
                    // Attach dragging via a given handle
                    function attachDrag(el, handle) {
                        let isDragging = false, startX, startY, startLeft, startTop;
                        handle.addEventListener('mousedown', function(e) {
                            // For images, convert to absolute if needed
                            if (!el.classList.contains('textbox')) {
                                ...
                                // For textbox, dragging via handle converts it to absolute
                                if (!el.style.position || el.style.position === 'static') {
                                    let rect = el.getBoundingClientRect();
                                    el.style.position = 'absolute';
                                    el.style.left = rect.left + 'px';
                    ...
                    // Apply interactive behavior
                    function makeInteractive(el) {
                        if (el.classList.contains('textbox')) {
                            // Add a drag handle if not present
                            ...
                            // Allow in-box text editing and limit select-all to the box
                            ...
                            // For images, attach drag on the element itself
                            attachDrag(el, el);
                        }
                        attachResize(el);
                    }
                    ...
                    // Context menu for text wrap options (excluding Text Over)
                            ...
                            menu.querySelector('#menu-resize').addEventListener('click', function() {
                                ...
                                if (handle) {
                                    let evt = new MouseEvent('mousedown', {
                                        clientX: parseInt(target.style.left || 0) + target.offsetWidth,
                                        clientY: parseInt(target.style.top || 0) + target.offsetHeight,
                                        bubbles: true,
                                        cancelable: true
                                    });
