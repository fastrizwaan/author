...
<body><p>&nbsp;</p></body>
    ...
    def on_webview_load(self, webview, load_event):
        if load_event == WebKit.LoadEvent.FINISHED:
            ...
            # Enhanced script with free dragging, resizing, and proper image handling
            enhanced_script = """
                (function() {
                    // Debounce function to notify content changes
                    ...
                    new MutationObserver(notifyChange).observe(document.body, {
                    ...
                    document.querySelectorAll('.resizable-image').forEach(function(img) {
                    ...
                    function makeInteractive(el) {
                        ...
                        el.addEventListener('mousedown', function(e) {
                            ...
                            // Convert from static/float layout to absolute positioning if needed
                        ...
                        // For images, ensure default drag is disabled
                        if (el.classList.contains('resizable-image')) {
                            el.setAttribute('draggable', 'false');
                            el.addEventListener('dragstart', function(e){ e.preventDefault(); });
                        }
                    }
                    
                    // Apply interactive behavior to existing elements
                    document.querySelectorAll('.textbox, .resizable-image').forEach(makeInteractive);
                        ...
                        mutations.forEach(function(mutation) {
                            if (mutation.addedNodes) {
                                mutation.addedNodes.forEach(function(node) {
                                    if (node.nodeType === 1 && (node.classList.contains('textbox') || node.classList.contains('resizable-image'))) {
                    ...
                    // Context menu for text wrap options
                    document.body.addEventListener('contextmenu', function(e) {
                        let target = e.target.closest('.textbox, .resizable-image');
                        if (target) {
                            e.preventDefault();
                            ...
                            menu.innerHTML = `
                                ...
                                <div id="menu-text-left">Text Around Left</div>
                                <div id="menu-text-right">Text Around Right</div>
                                <div id="menu-text-over">Text Over</div>
                            `;
                            document.body.appendChild(menu);
                            menu.querySelector('#menu-move').addEventListener('click', function() {
                                target.style.position = 'absolute';
                                target.style.float = 'none';
                                document.body.removeChild(menu);
                            });
                            menu.querySelector('#menu-resize').addEventListener('click', function() {
                                // Focus the resize handle by simulating a mousedown on it
                                let handle = target.querySelector('.resize-handle');
                                if (handle) {
                                    let event = new MouseEvent('mousedown', {
                                        clientX: parseInt(target.style.left) + target.offsetWidth,
                                        clientY: parseInt(target.style.top) + target.offsetHeight,
                                        bubbles: true,
                                        cancelable: true
                                    });
                            ...
                            menu.querySelector('#menu-text-left').addEventListener('click', function() {
                            ...
                            menu.querySelector('#menu-text-right').addEventListener('click', function() {
                                let rect = target.getBoundingClientRect();
                                target.style.position = 'static';
                            ...
                            menu.querySelector('#menu-text-over').addEventListener('click', function() {
                                target.style.position = 'absolute';
                                target.style.float = 'none';
                                document.body.removeChild(menu);
                            });
                        ...
                        if (!document.getElementById(styleId)) {
                            ...
                            style.textContent = `
                                @media screen {
                                    ...
                                    .textbox { background-color: #333 !important; color: #e0e0e0 !important; }
                                    .context-menu { background: #333 !important; color: #e0e0e0 !important; border: 1px solid #555; }
                                    .context-menu div:hover { background: #444 !important; }
                                }
                            `;
                            document.head.appendChild(style);
                        }
