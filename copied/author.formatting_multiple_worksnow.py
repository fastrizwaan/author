...
################ /on Close clicked #####################
    def apply_persistent_formatting(self, format_type, enable):
        """Apply or remove formatting so that subsequently typed text adopts the style."""
        # Update the state based on the format_type
        if format_type == 'bold':
            self.is_bold = enable
        ...
        # Construct the combined text-decoration style
        decorations = []
        ...
        script = f"""
            (function() {{
                ...
                let range;
                if (!sel.rangeCount) {{
                    // No selection: position cursor in a suitable place
                    let p = document.querySelector('p') || document.createElement('p');
                    if (!p.parentNode) {{
                        document.body.appendChild(p);
                        p.innerHTML = '\u200B'; // Zero-width space
                ...
                if (range.collapsed) {{
                    // Insertion point: wrap with a span for persistent formatting
                    let span = document.createElement('span');
                    span.style.textDecoration = '{decoration_style}';
                    {(f"span.style.fontWeight = 'bold';" if self.is_bold else "")}
                    {(f"span.style.fontStyle = 'italic';" if self.is_italic else "")}
                    ...
                    // Selection exists: apply commands and combine decorations
                    let currentBold = document.queryCommandState('bold');
                    let currentItalic = document.queryCommandState('italic');
                    if ({str(self.is_bold).lower()} !== currentBold) {{
                        document.execCommand('bold', false, null);
                    }}
                    if ({str(self.is_italic).lower()} !== currentItalic) {{
                        document.execCommand('italic', false, null);
                    }}
                    document.execCommand('underline', false, null); // Refresh underline
                    document.execCommand('strikethrough', false, null); // Refresh strikethrough
                    let selectedSpan = range.commonAncestorContainer.nodeType === 1 ? 
                        range.commonAncestorContainer : 
                        range.commonAncestorContainer.parentElement;
                    selectedSpan.style.textDecoration = '{decoration_style}';
                }}
                console.log('Applied formatting - bold: {self.is_bold}, italic: {self.is_italic}, text-decoration: {decoration_style}');
            }})();
        """
        self.exec_js(script)
        ...
        script = f"""
            (function() {{
                ...
                if (range.collapsed) {{
                    ...
                    // Selection exists: apply commands directly
                    // First, remove all formatting that should be off
                    if (!{str(self.is_bold).lower()}) {{
                        if (document.queryCommandState('bold')) {{
                            document.execCommand('bold', false, null);
                        }}
                    }}
                    if (!{str(self.is_italic).lower()}) {{
                        if (document.queryCommandState('italic')) {{
                            document.execCommand('italic', false, null);
                        }}
                    }}
                    
                    // Then apply formatting that should be on
                    if ({str(self.is_bold).lower()}) {{
                        if (!document.queryCommandState('bold')) {{
                            document.execCommand('bold', false, null);
                        }}
                    ...
                    let selectedElement = range.commonAncestorContainer.nodeType === 1 ? 
                        range.commonAncestorContainer : 
                        range.commonAncestorContainer.parentElement;
                    selectedElement.style.textDecoration = '{decoration_style}';
                }}
                console.log('Applied formatting - bold: {self.is_bold}, italic: {self.is_italic}, text-decoration: {decoration_style}');
            }})();
        """
    ...
    def apply_persistent_formatting(self, format_type, enable):
        ...
        script = f"""
            (function() {{
                ...
                // First, ensure we have a valid selection or cursor position
                if (!sel.rangeCount) {{
                    let p = document.querySelector('p') || document.createElement('p');
                    if (!p.parentNode) {{
                        document.body.appendChild(p);
                ...
                // Handle each format type explicitly
                if ('{format_type}' === 'bold') {{
                    if ({str(enable).lower()}) {{
                    ...
                    // For underline, we need a special approach
                    if ({str(enable).lower()}) {{
                        ...
                        // First apply and then remove to ensure state is consistent
                        document.execCommand('underline', false, null);
                        document.execCommand('underline', false, null);
                    }}
                }}
                else if ('{format_type}' === 'strikethrough') {{
                    // For strikethrough, similar approach to underline
                    if ({str(enable).lower()}) {{
                        if (!document.queryCommandState('strikethrough')) {{
                            document.execCommand('strikethrough', false, null);
                        }}
                    }} else {{
                        // Force remove strikethrough
                        // First apply and then remove to ensure state is consistent
                        document.execCommand('strikethrough', false, null);
                        document.execCommand('strikethrough', false, null);
                    }}
                ...
                // Create a marker span for cursor position to maintain formatting for future typing
                let range = sel.getRangeAt(0);
                if (range.collapsed) {{
                    let markerSpan = document.createElement('span');
                    markerSpan.classList.add('format-marker');
                    markerSpan.style.fontWeight = {("'bold'" if self.is_bold else "'normal'")};
                    markerSpan.style.fontStyle = {("'italic'" if self.is_italic else "'normal'")};
                    
                    // Set text decoration explicitly
                    let decorations = [];
                    if ({str(self.is_underline).lower()}) decorations.push('underline');
                    if ({str(self.is_strikethrough).lower()}) decorations.push('line-through');
                    markerSpan.style.textDecoration = decorations.length ? decorations.join(' ') : 'none';
                    
                    // Insert zero-width space as marker
                    markerSpan.innerHTML = '\u200B';
                    range.insertNode(markerSpan);
                    
                    // Position cursor inside the marker
                    range = document.createRange();
                    range.setStart(markerSpan.firstChild, 1);
                    range.setEnd(markerSpan.firstChild, 1);
                    sel.removeAllRanges();
                    sel.addRange(range);
                }}
                
                console.log('Formatting applied: {format_type} = {enable}');
            }})();
        """
        self.exec_js(script)
############### c with the above
    def apply_persistent_formatting(self, format_type, enable):
        ...
        script = f"""
            (function() {{
                ...
                // Remove any existing marker to avoid duplicate nodes.
                let oldMarker = document.querySelector('.format-marker');
                if (oldMarker) {{
                ...
                if ('{format_type}' === 'bold') {{
                    ...
                    if ({str(enable).lower()}) {{
                        ...
                        // Remove underline by toggling twice.
                        document.execCommand('underline', false, null);
                        document.execCommand('underline', false, null);
                    }}
                }}
                    ...
                    if ({str(enable).lower()}) {{
                        ...
                        // Remove strikethrough by toggling twice.
                        document.execCommand('strikethrough', false, null);
                        document.execCommand('strikethrough', false, null);
                    }}
                }}

                // Insert a marker span to maintain formatting for future text.
                let range = sel.getRangeAt(0);
                if (range.collapsed) {{
                    let markerSpan = document.createElement('span');
                    markerSpan.classList.add('format-marker');
    ...
    def apply_persistent_formatting(self, format_type, enable):
        ...
        script = f"""
            (function() {{
                ...
                // Ensure a valid, collapsed selection.
                if (!sel.rangeCount) {{
                    let p = document.querySelector('p') || document.createElement('p');
                    if (!p.parentNode) {{
                        document.body.appendChild(p);
                ...
                // Get current range and ensure it is collapsed.
                let range = sel.getRangeAt(0);
                if (!range.collapsed) {{
                    range.collapse(true);
                }}

                // Create a marker span to preserve formatting.
                let markerSpan = document.createElement('span');
                markerSpan.classList.add('format-marker');
                markerSpan.style.fontWeight = {("'bold'" if self.is_bold else "'normal'")};
                markerSpan.style.fontStyle = {("'italic'" if self.is_italic else "'normal'")};
                ...
                // Insert marker without deleting existing content.
                range.insertNode(markerSpan);
...
############# cl with the above works but floods span and zero width
    def apply_persistent_formatting(self, format_type, enable):
        """Apply or remove formatting so that subsequently typed text adopts the style."""
        if format_type == 'bold':
            self.is_bold = enable
        ...
        script = f"""
            (function() {{
                ...
                if (range.collapsed) {{
                    // Collapsed range: split text and apply formatting for new text
                    let container = range.startContainer;
                    let offset = range.startOffset;

                    if (container.nodeType === 3 && offset < container.length) {{
                        // Split text node at cursor
                        let beforeText = container.textContent.substring(0, offset);
                        let afterText = container.textContent.substring(offset);
                        ...
                        // Apply formatting commands to ensure persistence
                        range.selectNodeContents(span);
                        sel.removeAllRanges();
                        sel.addRange(range);
                        if ({str(self.is_bold).lower()} !== document.queryCommandState('bold')) {{
                            document.execCommand('bold', false, null);
                        }}
                        if ({str(self.is_italic).lower()} !== document.queryCommandState('italic')) {{
                            document.execCommand('italic', false, null);
                        }}
                        if ({str(self.is_underline).lower()} !== document.queryCommandState('underline')) {{
                            document.execCommand('underline', false, null);
                        }}
                        if ({str(self.is_strikethrough).lower()} !== document.queryCommandState('strikethrough')) {{
                            document.execCommand('strikethrough', false, null);
                        }}

                        // Reset range to insertion point
                        range.setStart(span.firstChild, afterText ? 0 : 1);
                        range.collapse(true);
                        sel.removeAllRanges();
                        sel.addRange(range);
                    }} else {{
                        // No split needed: insert new span
                        let span = document.createElement('span');
                        span.style.fontWeight = {("'bold'" if self.is_bold else "'normal'")};
                        span.style.fontStyle = {("'italic'" if self.is_italic else "'normal'")};
                        span.style.textDecoration = '{decoration_style}';
                    ...
                    // Selection exists: apply to selected text only
                    if ({str(self.is_bold).lower()} !== document.queryCommandState('bold')) {{
                        document.execCommand('bold', false, null);
                    }}
                    if ({str(self.is_italic).lower()} !== document.queryCommandState('italic')) {{
                    ...
                    // Ensure combined text-decoration
                    let contents = range.extractContents();
...
######### g with the above works nicely can be used, but a little feels odd
    def apply_persistent_formatting(self, format_type, enable):
        ...
        script = f"""
            (function() {{
                ...
                if (range.collapsed) {{
                    // Collapsed range: apply formatting for new text without extra spans
                    if ('{format_type}' === 'bold') {{
                    ...
                    // Insert a temporary marker only if no text follows
                    //let container = range.startContainer;
                    ...
                    // Consolidate styles for selection
                    let contents = range.extractContents();
                    if (contents.textContent.length > 0) {{
                        let span = document.createElement('span');
                        span.style.fontWeight = {("'bold'" if self.is_bold else "'normal'")};
...
######### g refeined above
    def apply_persistent_formatting(self, format_type, enable):
        ...
        """
        self.exec_js(script)
############# The above applies but does not remove

# ### This one formats/unformats whole line;
#     def apply_persistent_formatting(self, format_type, enable):
...
#                 let range;
#                 if (!sel.rangeCount) {{
#                     // No selection: position cursor and prepare formatting
#                     let p = document.querySelector('p') || document.createElement('p');
#                     if (!p.parentNode) {{
#                         document.body.appendChild(p);
#                         p.innerHTML = '\u200B';
...
#                 if (range.collapsed) {{
#                     // Insertion point: manage a formatting span
#                     let container = range.startContainer;
#                     let parentSpan = container.nodeType === 1 ? container : container.parentElement;
#                     if (parentSpan.tagName === 'SPAN' && parentSpan.style.textDecoration) {{
#                         // Update existing span
#                         parentSpan.style.textDecoration = '{decoration_style}';
#                         {(f"parentSpan.style.fontWeight = 'bold';" if self.is_bold else "parentSpan.style.fontWeight = 'normal';")}
#                         {(f"parentSpan.style.fontStyle = 'italic';" if self.is_italic else "parentSpan.style.fontStyle = 'normal';")}
#                         if (!parentSpan.style.textDecoration && !parentSpan.style.fontWeight && !parentSpan.style.fontStyle) {{
#                             // Remove span if no styles remain
#                             let text = parentSpan.textContent;
#                             parentSpan.replaceWith(document.createTextNode(text));
#                             range.setStartAfter(parentSpan.previousSibling || p);
#                             range.collapse(true);
#                             sel.removeAllRanges();
#                             sel.addRange(range);
#                         }}
...
#                     }}
#                 }} else {{
#                     // Selection exists: apply or remove styles
#                     let currentBold = document.queryCommandState('bold');
#                     let currentItalic = document.queryCommandState('italic');
#                     let currentUnderline = document.queryCommandState('underline');
#                     let currentStrikethrough = document.queryCommandState('strikethrough');
...
#                         document.execCommand('italic', false, null);
#                     }}
#                     if ({str(self.is_underline).lower()} !== currentUnderline) {{
#                         document.execCommand('underline', false, null);
#                     }}
#                     if ({str(self.is_strikethrough).lower()} !== currentStrikethrough) {{
#                         document.execCommand('strikethrough', false, null);
#                     }}

#                     // Ensure combined text-decoration for selection
#                     let ancestor = range.commonAncestorContainer;
#                     let target = ancestor.nodeType === 1 ? ancestor : ancestor.parentElement;
#                     if (target.tagName === 'SPAN') {{
#                         target.style.textDecoration = '{decoration_style}';
#                         {(f"target.style.fontWeight = 'bold';" if self.is_bold else "target.style.fontWeight = 'normal';")}
#                         {(f"target.style.fontStyle = 'italic';" if self.is_italic else "target.style.fontStyle = 'normal';")}
#                         if ('{decoration_style}' === 'none' && !target.style.fontWeight && !target.style.fontStyle) {{
#                             let content = target.textContent;
#                             target.replaceWith(document.createTextNode(content));
#                         }}
#                     }}
#                 }}
#                 console.log('Applied formatting - bold: {self.is_bold}, italic: {self.is_italic}, underline: {self.is_underline}, strikethrough: {self.is_strikethrough}');
...
#         """
#         self.exec_js(script)
################  this below one unformats pervious world
    # def apply_persistent_formatting(self, format_type, enable):
    #     """Apply or remove formatting for new text or selected text only."""
    #     # Update the state based on the format_type
    #     if format_type == 'bold':
    #         self.is_bold = enable
    #     elif format_type == 'italic':
    ...
    #             let range;
    #             if (!sel.rangeCount) {{
    #                 // No selection: position cursor and apply formatting at insertion point
    #                 let p = document.querySelector('p') || document.createElement('p');
    #                 if (!p.parentNode) {{
    #                     document.body.appendChild(p);
    #                     p.innerHTML = '\u200B';
    ...
    #             if (range.collapsed) {{
    #                 // Insertion point: insert a new span for new text
    #                 let span = document.createElement('span');
    #                 span.style.textDecoration = '{decoration_style}';
    #                 {(f"span.style.fontWeight = 'bold';" if self.is_bold else "span.style.fontWeight = 'normal';")}
    #                 {(f"span.style.fontStyle = 'italic';" if self.is_italic else "span.style.fontStyle = 'normal';")}
    ...
    #                 // Clean up any parent span that might interfere
    #                 let parentSpan = span.parentElement;
    #                 if (parentSpan.tagName === 'SPAN' && parentSpan !== span) {{
    #                     parentSpan.style.textDecoration = 'inherit';
    #                     parentSpan.style.fontWeight = 'inherit';
    #                     parentSpan.style.fontStyle = 'inherit';
    #                 }}
    #             }} else {{
    #                 // Selection exists: wrap only the selected text
    #                 let currentBold = document.queryCommandState('bold');
    #                 let currentItalic = document.queryCommandState('italic');
    #                 let currentUnderline = document.queryCommandState('underline');
    #                 let currentStrikethrough = document.queryCommandState('strikethrough');

    #                 // Apply/remove individual styles
    #                 if ({str(self.is_bold).lower()} !== currentBold) {{
    #                     document.execCommand('bold', false, null);
    #                 }}
    #                 if ({str(self.is_italic).lower()} !== currentItalic) {{
    ...
    #                 // Wrap selection in a span with combined styles
    #                 let contents = range.extractContents();
    #                 if (contents.textContent.length > 0) {{
    #                     let span = document.createElement('span');
    #                     span.style.textDecoration = '{decoration_style}';
    ...
    #                     // Clean up if no styles remain
    #                     if ('{decoration_style}' === 'none' && !{self.is_bold} && !{self.is_italic}) {{
